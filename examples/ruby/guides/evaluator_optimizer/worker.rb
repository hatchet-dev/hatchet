...
GENERATOR_WF.task(:generate_draft) do |input, ctx|
...
EVALUATOR_WF.task(:evaluate_draft) do |input, ctx|
  mock_evaluate(input["draft"])
end

# > Step 02 Optimization Loop
OPTIMIZER_TASK = HATCHET.durable_task(name: "EvaluatorOptimizer", execution_timeout: "5m") do |input, ctx|
  max_iterations = 3
  threshold = 0.8
  draft = ""
  feedback = ""
  ...
  max_iterations.times do |i|
    ...
    if evaluation["score"] >= threshold
      next { "draft" => draft, "iterations" => i + 1, "score" => evaluation["score"] }
    end
...
def main
  # > Step 03 Run Worker
  worker = HATCHET.worker("evaluator-optimizer-worker", slots: 5, workflows: [GENERATOR_WF, EVALUATOR_WF, OPTIMIZER_TASK])
  worker.start
end
