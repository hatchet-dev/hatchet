...
APPROVAL_TASK = HATCHET.durable_task(name: "ApprovalTask") do |input, ctx|
...
def wait_for_approval(ctx, proposed_action)
  ...
  approval["approved"] ? { "status" => "approved", "action" => proposed_action } : { "status" => "rejected", "reason" => approval["reason"].to_s }
end
