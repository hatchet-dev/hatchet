...
CONTENT_WF.task(:generate_content) do |input, ctx|
  { "content" => mock_generate_content(input["message"]) }
end

SAFETY_WF.task(:safety_check) do |input, ctx|
  mock_safety_check(input["message"])
end

EVALUATOR_WF.task(:evaluate_content) do |input, ctx|
  mock_evaluate_content(input["content"])
end

# > Step 02 Sectioning
SECTIONING_TASK = HATCHET.durable_task(name: "ParallelSectioning", execution_timeout: "2m") do |input, ctx|
  threads = []
...
# > Step 03 Voting
VOTING_TASK = HATCHET.durable_task(name: "ParallelVoting", execution_timeout: "3m") do |input, ctx|
...
def main
  # > Step 04 Run Worker
  worker = HATCHET.worker("parallelization-worker", slots: 10, workflows: [CONTENT_WF, SAFETY_WF, EVALUATOR_WF, SECTIONING_TASK, VOTING_TASK])
  worker.start
end
