...
CLASSIFY_WF.task(:classify_message) do |input, ctx|
  { "category" => mock_classify(input["message"]) }
end

# > Step 02 Specialist Tasks
SUPPORT_WF.task(:handle_support) do |input, ctx|
  { "response" => mock_reply(input["message"], "support"), "category" => "support" }
end

SALES_WF.task(:handle_sales) do |input, ctx|
  { "response" => mock_reply(input["message"], "sales"), "category" => "sales" }
end

DEFAULT_WF.task(:handle_default) do |input, ctx|
  { "response" => mock_reply(input["message"], "other"), "category" => "other" }
end

# > Step 03 Router Task
ROUTER_TASK = HATCHET.durable_task(name: "MessageRouter", execution_timeout: "2m") do |input, ctx|
...
def main
  # > Step 04 Run Worker
  worker = HATCHET.worker("routing-worker", slots: 5, workflows: [CLASSIFY_WF, SUPPORT_WF, SALES_WF, DEFAULT_WF, ROUTER_TASK])
  worker.start
end
