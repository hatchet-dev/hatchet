...
SCRAPE_WF.task(:scrape_url, execution_timeout: "2m", retries: 2) do |input, ctx|
  mock_scrape(input["url"])
end

# > Step 02 Process Content
PROCESS_WF.task(:process_content) do |input, ctx|
...
CRON_WF.task(:scheduled_scrape) do |input, ctx|
  urls = %w[
