...
DETAIL_STEP1 = RUN_DETAIL_TEST_WORKFLOW.task(:step1) do |_input, _ctx|
  { "random_number" => rand(1..100) }
end

RUN_DETAIL_TEST_WORKFLOW.task(:cancel_step) do |_input, ctx|
  ctx.cancel
  10.times { sleep 1 }
end

RUN_DETAIL_TEST_WORKFLOW.task(:fail_step) do |_input, _ctx|
  raise "Intentional Failure"
end

DETAIL_STEP2 = RUN_DETAIL_TEST_WORKFLOW.task(:step2) do |_input, _ctx|
  sleep 5
  { "random_number" => rand(1..100) }
end

RUN_DETAIL_TEST_WORKFLOW.task(:step3, parents: [DETAIL_STEP1, DETAIL_STEP2]) do |_input, ctx|
