...
func replayExample() { //nolint:unused
	// > Setup
	...
	_, err = client.Runs().Replay(ctx, rest.V1ReplayTaskRequest{
		ExternalIds: &runIds,
	})
	if err != nil {
		log.Fatalf("failed to replay runs: %v", err)
	}
	...
	if err != nil {
		log.Fatalf("failed to replay runs by filters: %v", err)
	}
}
