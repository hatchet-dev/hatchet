...
def new_client(*opts_functions):
    ...
    # shared_opts = {'tenant_id': opts.tenant_id, 'logger': opts.logger, 'validator': opts.validator}
    # admin_client = new_admin(conn, shared_opts)
    # dispatcher_client = new_dispatcher(conn, shared_opts)
    event_client = new_event(conn, config)
    admin_client = new_admin(conn, config)
    dispatcher_client = new_dispatcher(conn, config)

    # if opts.init_workflows:
    #     init_workflows(opts.files_loader, admin_client)
