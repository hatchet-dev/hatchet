...
const upsertAPIToken = `-- name: UpsertAPIToken :one
INSERT INTO "APIToken" (
    "id",
    "createdAt",
    "updatedAt",
...
type UpsertAPITokenParams struct {
	ID        pgtype.UUID      `json:"id"`
	TenantId  pgtype.UUID      `json:"tenantId"`
	Name      pgtype.Text      `json:"name"`
	Expiresat pgtype.Timestamp `json:"expiresat"`
...
func (q *Queries) UpsertAPIToken(ctx context.Context, db DBTX, arg UpsertAPITokenParams) (*APIToken, error) {
	row := db.QueryRow(ctx, upsertAPIToken,
		arg.ID,
		arg.TenantId,
		arg.Name,
		arg.Expiresat,
