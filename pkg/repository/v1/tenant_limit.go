...
func (t *tenantLimitRepository) DefaultLimits() []Limit {
	return []Limit{
		{
			Resource:         sqlcv1.LimitResourceWORKFLOWRUN,
			Limit:            int32(t.config.DefaultWorkflowRunLimit),      // nolint: gosec
			Alarm:            int32(t.config.DefaultWorkflowRunAlarmLimit), // nolint: gosec
			Window:           &t.config.DefaultWorkflowRunWindow,
