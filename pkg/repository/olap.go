...
type OLAPRepository interface {
	...
	GetTaskPointMetrics(ctx context.Context, tenantId string, startTimestamp *time.Time, endTimestamp *time.Time, bucketInterval time.Duration, workflowIds []uuid.UUID, additionalMetadata map[string]string) ([]*sqlcv1.GetTaskPointMetricsRow, error)
...
func (r *OLAPRepositoryImpl) GetTaskPointMetrics(ctx context.Context, tenantId string, startTimestamp *time.Time, endTimestamp *time.Time, bucketInterval time.Duration, workflowIds []uuid.UUID, additionalMetadata map[string]string) ([]*sqlcv1.GetTaskPointMetricsRow, error) {
	...
	if len(additionalMetadata) > 0 {
		additionalMetaKeys = make([]string, 0, len(additionalMetadata))
		additionalMetaValues = make([]string, 0, len(additionalMetadata))
		for k, v := range additionalMetadata {
			additionalMetaKeys = append(additionalMetaKeys, k)
			additionalMetaValues = append(additionalMetaValues, v)
		}
	}
