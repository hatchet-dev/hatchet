...
// AcquireConnectionWithStatementTimeout acquires a connection from the pool and overwrites the statement timeout on it.
func AcquireConnectionWithStatementTimeout(ctx context.Context, pool *pgxpool.Pool, l *zerolog.Logger, timeoutMs int) (*pgx.Conn, func(), error) {
	conn, err := AcquirePoolConnectionWithStatementTimeout(ctx, pool, l, timeoutMs)
	if err != nil {
		return nil, nil, err
	}
