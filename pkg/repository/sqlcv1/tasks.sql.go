...
const readWorkflowRunTerminalStatesByExternalIds = `-- name: ReadWorkflowRunTerminalStatesByExternalIds :many
WITH input AS (
    SELECT
    ...
    SELECT external_id, id, inserted_at, retry_count FROM dag_tasks
    UNION ALL
    SELECT external_id, id, inserted_at, retry_count FROM task_only
), terminal_tasks AS (
    SELECT
        at.external_id,
        EXISTS (
...
type ReadWorkflowRunTerminalStatesByExternalIdsParams struct {
	Externalids []uuid.UUID `json:"externalids"`
	Tenantid    uuid.UUID   `json:"tenantid"`
}

type ReadWorkflowRunTerminalStatesByExternalIdsRow struct {
	ExternalID  uuid.UUID `json:"external_id"`
	AllTerminal bool      `json:"all_terminal"`
	TaskCount   int64     `json:"task_count"`
}

func (q *Queries) ReadWorkflowRunTerminalStatesByExternalIds(ctx context.Context, db DBTX, arg ReadWorkflowRunTerminalStatesByExternalIdsParams) ([]*ReadWorkflowRunTerminalStatesByExternalIdsRow, error) {
	rows, err := db.Query(ctx, readWorkflowRunTerminalStatesByExternalIds, arg.Externalids, arg.Tenantid)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []*ReadWorkflowRunTerminalStatesByExternalIdsRow
	for rows.Next() {
		var i ReadWorkflowRunTerminalStatesByExternalIdsRow
		if err := rows.Scan(&i.ExternalID, &i.AllTerminal, &i.TaskCount); err != nil {
			return nil, err
		}
		items = append(items, &i)
	}
