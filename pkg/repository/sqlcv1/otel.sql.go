...
type ListSpansByTraceIDsRow struct {
...
func (q *Queries) ListSpansByTraceIDs(ctx context.Context, db DBTX, arg ListSpansByTraceIDsParams) ([]*ListSpansByTraceIDsRow, error) {
	rows, err := db.Query(ctx, listSpansByTraceIDs, arg.Tenantid, arg.Traceids)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []*ListSpansByTraceIDsRow
	for rows.Next() {
		var i ListSpansByTraceIDsRow
		if err := rows.Scan(
			&i.TraceID,
			&i.SpanID,
			&i.ParentSpanID,
