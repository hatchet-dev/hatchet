...
const createTaskEventsOLAP = `-- name: CreateTaskEventsOLAP :exec
WITH inputs AS (
    SELECT
        UNNEST($1::UUID[]) AS tenant_id,
        UNNEST($2::BIGINT[]) AS task_id,
        UNNEST($3::TIMESTAMPTZ[]) AS task_inserted_at,
        UNNEST($4::v1_task_event_type_olap[]) AS event_type,
        UNNEST($5::UUID[]) AS workflow_id,
        UNNEST($6::TIMESTAMPTZ[]) AS event_timestamp,
        UNNEST($7::v1_readable_status_olap[]) AS readable_status,
        UNNEST($8::INTEGER[]) AS retry_count,
        UNNEST($9::TEXT[]) AS error_message,
        UNNEST($10::JSONB[]) AS output,
        UNNEST($11::UUID[]) AS worker_id,
        UNNEST($12::TEXT[]) AS additional__event_data,
        UNNEST($13::TEXT[]) AS additional__event_message,
        UNNEST($14::UUID[]) AS external_id,
        UNNEST($15::INTEGER[]) AS durable_invocation_count
)
...
type CreateTaskEventsOLAPParams struct {
	...
	Workflowids             []uuid.UUID            `json:"workflowids"`
	Eventtimestamps         []pgtype.Timestamptz   `json:"eventtimestamps"`
	...
	Additionaleventdatas    []string               `json:"additionaleventdatas"`
	Additionaleventmessages []string               `json:"additionaleventmessages"`
	Externalids             []uuid.UUID            `json:"externalids"`
	Durableinvocationcounts []int32                `json:"durableinvocationcounts"`
}

func (q *Queries) CreateTaskEventsOLAP(ctx context.Context, db DBTX, arg CreateTaskEventsOLAPParams) error {
	_, err := db.Exec(ctx, createTaskEventsOLAP,
		arg.Tenantids,
		arg.Taskids,
		arg.Taskinsertedats,
		arg.Eventtypes,
		...
		arg.Outputs,
		arg.Workerids,
		arg.Additionaleventdatas,
		arg.Additionaleventmessages,
		arg.Externalids,
		arg.Durableinvocationcounts,
	)
	return err
}
...
func (q *Queries) UpdateDAGStatusesFromMQ(ctx context.Context, db DBTX, arg UpdateDAGStatusesFromMQParams) ([]*UpdateDAGStatusesFromMQRow, error) {
	rows, err := db.Query(ctx, updateDAGStatusesFromMQ, arg.Tenantids, arg.Dagids, arg.Daginsertedats)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
