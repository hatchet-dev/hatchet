package prometheus

import (
	"math"
	"testing"
	"time"

	"github.com/stretchr/testify/assert"
)

// fakeClock lets tests control which per-second bucket receives observations.
type fakeClock struct {
	t time.Time
}

func (c *fakeClock) now() time.Time {
	return c.t
}

func (c *fakeClock) advance(d time.Duration) {
	c.t = c.t.Add(d)
}

func newTestWindow(windowSecs, perSecondCap int) (*quantileWindow, *fakeClock, *int) {
	clock := &fakeClock{t: time.Unix(1_000_000, 0)}
	drops := 0

	w := newQuantileWindow(windowSecs, perSecondCap, clock.now, func() { drops++ })
	return w, clock, &drops
}

func TestQuantileWindowKnownDistribution(t *testing.T) {
	w, _, _ := newTestWindow(30, 1000)

	// 1..100: nearest-rank p50 is the 50th value, p99 the 99th.
	for i := 1; i <= 100; i++ {
		w.Observe(float64(i))
	}

	p50, p99 := w.quantiles()
	assert.Equal(t, 50.0, p50)
	assert.Equal(t, 99.0, p99)
}

func TestQuantileWindowSpansSeconds(t *testing.T) {
	w, clock, _ := newTestWindow(30, 1000)

	// Samples spread over several seconds all count while inside the window.
	for i := 1; i <= 100; i++ {
		w.Observe(float64(i))
		if i%10 == 0 {
			clock.advance(time.Second)
		}
	}

	p50, p99 := w.quantiles()
	assert.Equal(t, 50.0, p50)
	assert.Equal(t, 99.0, p99)
}

func TestQuantileWindowExpiry(t *testing.T) {
	w, clock, _ := newTestWindow(30, 1000)

	w.Observe(100.0) // old sample

	clock.advance(10 * time.Second)
	w.Observe(1.0)
	w.Observe(2.0)
	w.Observe(3.0)

	// Both generations in the window: the old 100 is the max, so p99 = 100.
	_, p99 := w.quantiles()
	assert.Equal(t, 100.0, p99)

	// 25s after the old sample it is still in the 30s window...
	clock.advance(15 * time.Second)
	_, p99 = w.quantiles()
	assert.Equal(t, 100.0, p99)

	// ...but 35s after, only the newer samples remain.
	clock.advance(10 * time.Second)
	p50, p99 := w.quantiles()
	assert.Equal(t, 2.0, p50)
	assert.Equal(t, 3.0, p99)

	// Once everything ages out the window is empty again.
	clock.advance(30 * time.Second)
	p50, p99 = w.quantiles()
	assert.True(t, math.IsNaN(p50))
	assert.True(t, math.IsNaN(p99))
}

func TestQuantileWindowEmpty(t *testing.T) {
	w, _, _ := newTestWindow(30, 1000)

	p50, p99 := w.quantiles()
	assert.True(t, math.IsNaN(p50), "empty window must not report a number")
	assert.True(t, math.IsNaN(p99), "empty window must not report a number")
}

func TestQuantileWindowPerSecondCap(t *testing.T) {
	w, clock, drops := newTestWindow(30, 3)

	// Only the first 3 samples of the second are kept; the rest are dropped
	// and counted.
	for i := 1; i <= 5; i++ {
		w.Observe(float64(i))
	}

	assert.Equal(t, 2, *drops)

	p50, p99 := w.quantiles()
	assert.Equal(t, 2.0, p50)
	assert.Equal(t, 3.0, p99)

	// The cap is per second: the next second accepts samples again.
	clock.advance(time.Second)
	w.Observe(10.0)

	_, p99 = w.quantiles()
	assert.Equal(t, 10.0, p99)
	assert.Equal(t, 2, *drops, "no additional drops in the fresh second")
}

func TestQuantileWindowRingSlotReuse(t *testing.T) {
	windowSecs := 5
	w, clock, _ := newTestWindow(windowSecs, 1000)

	w.Observe(100.0)

	// windowSecs later the same ring slot is claimed by a new second; the old
	// bucket must be discarded, not mixed in.
	clock.advance(time.Duration(windowSecs) * time.Second)
	w.Observe(1.0)

	p50, p99 := w.quantiles()
	assert.Equal(t, 1.0, p50)
	assert.Equal(t, 1.0, p99)
}

func TestQuantileSortedNearestRank(t *testing.T) {
	assert.Equal(t, 1.0, quantileSorted([]float64{1}, 0.5))
	assert.Equal(t, 1.0, quantileSorted([]float64{1}, 0.99))
	assert.Equal(t, 1.0, quantileSorted([]float64{1, 2}, 0.5))
	assert.Equal(t, 2.0, quantileSorted([]float64{1, 2}, 0.99))
}
