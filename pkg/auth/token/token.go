...
type JWTManager interface {
	GenerateTenantToken(ctx context.Context, tenantId, name string, _expiresAt *time.Time) (*Token, error)
	UpsertTenantToken(ctx context.Context, tenantId, name, id string, _expiresAt *time.Time) (string, error)
...
func (j *jwtManagerImpl) createToken(ctx context.Context, tenantId, name string, id *string, _expiresAt *time.Time) (*Token, error) {
	...
	tokenId, expiresAt, opts := j.getJWTOptionsForTenant(tenantId, id, _expiresAt)
...
func (j *jwtManagerImpl) GenerateTenantToken(ctx context.Context, tenantId, name string, _expiresAt *time.Time) (*Token, error) {
	token, err := j.createToken(ctx, tenantId, name, nil, _expiresAt)
	if err != nil {
		return nil, err
	}
...
func (j *jwtManagerImpl) UpsertTenantToken(ctx context.Context, tenantId, name, id string, _expiresAt *time.Time) (string, error) {
	token, err := j.createToken(ctx, tenantId, name, &id, _expiresAt)
	if err != nil {
		return "", err
	}
...
func (j *jwtManagerImpl) getJWTOptionsForTenant(tenantId string, id *string, _expiresAt *time.Time) (tokenId string, expiresAt time.Time, opts *jwt.RawJWTOptions) {
