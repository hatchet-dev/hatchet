...
func TestRoutingCacheRouteAndMissRefresh(t *testing.T) {
	...
	assert.Equal(t, sortedUnion([]string{prefixed(a.Namespace, "svc:a"), prefixed(b.Namespace, "svc:b")}), cache.ActionUnion())
...
func TestRoutingCacheRefreshAndUnion(t *testing.T) {
	...
	assert.Equal(t, []string{prefixed(a.Namespace, "svc:a")}, cache.ActionUnion(), "disabled endpoints are excluded from the union")
	...
	assert.Equal(t, []string{prefixed(a.Namespace, "svc:a"), prefixed(a.Namespace, "svc:a2")}, cache.ActionUnion())
