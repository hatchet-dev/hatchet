...
func newCancelQueuedExceptNewestStrategy(repo repository.ConcurrencyRepository, maxConcurrency int32) *ConcurrencyStrategy {
	return newTestStrategyKind(repo, maxConcurrency, sqlcv1.V1ConcurrencyStrategyCANCELQUEUEDEXCEPTNEWEST)
}
...
func TestCancelQueuedExceptNewest_SingleBatchKeepsOnlyNewestQueued(t *testing.T) {
	now := time.Now().UTC()
	future := now.Add(time.Hour)

	repo := &mockConcurrencyRepo{}
	c := newCancelQueuedExceptNewestStrategy(repo, 1)
...
func TestCancelQueuedExceptNewest_AcrossSuccessiveDecideCalls(t *testing.T) {
	now := time.Now().UTC()
	future := now.Add(time.Hour)
...
func TestCancelQueuedExceptNewest_PromotesSurvivorAsSoonAsRunningSlotFrees(t *testing.T) {
	now := time.Now().UTC()
	future := now.Add(time.Hour)
...
// At capacity, CANCEL_QUEUED_EXCEPT_NEWEST must never preempt running work, even across many arrivals -
// only the queued backlog is ever touched.
func TestCancelQueuedExceptNewest_NeverPreemptsRunning(t *testing.T) {
	now := time.Now().UTC()
	future := now.Add(time.Hour)
	...
	c := newCancelQueuedExceptNewestStrategy(repo, 2)
...
func TestCancelQueuedExceptNewest_KeepsNewestNWhenMaxRunsGreaterThanOne(t *testing.T) {
	now := time.Now().UTC()
	future := now.Add(time.Hour)
