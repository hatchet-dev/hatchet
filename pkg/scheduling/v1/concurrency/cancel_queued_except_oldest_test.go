...
func newCancelQueuedExceptOldestStrategy(repo repository.ConcurrencyRepository, maxConcurrency int32) *ConcurrencyStrategy {
	return newTestStrategyKind(repo, maxConcurrency, sqlcv1.V1ConcurrencyStrategyCANCELQUEUEDEXCEPTOLDEST)
}
...
func TestCancelQueuedExceptOldest_SingleBatchKeepsOnlyOldestQueued(t *testing.T) {
	now := time.Now().UTC()
	future := now.Add(time.Hour)

	repo := &mockConcurrencyRepo{}
	c := newCancelQueuedExceptOldestStrategy(repo, 1)
...
func TestCancelQueuedExceptOldest_AcrossSuccessiveDecideCalls(t *testing.T) {
	now := time.Now().UTC()
	future := now.Add(time.Hour)
...
// next decide() call.
func TestCancelQueuedExceptOldest_PromotesSurvivorAsSoonAsRunningSlotFrees(t *testing.T) {
	now := time.Now().UTC()
	future := now.Add(time.Hour)
...
// At capacity, CANCEL_QUEUED_EXCEPT_OLDEST must never preempt running work.
func TestCancelQueuedExceptOldest_NeverPreemptsRunning(t *testing.T) {
	now := time.Now().UTC()
	future := now.Add(time.Hour)
	...
	c := newCancelQueuedExceptOldestStrategy(repo, 2)
...
func TestCancelQueuedExceptOldest_KeepsOldestNWhenMaxRunsGreaterThanOne(t *testing.T) {
	now := time.Now().UTC()
	future := now.Add(time.Hour)
