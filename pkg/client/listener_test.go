...
func TestAddWorkflowRunDuringReconnectHandoffReachesReplacementStream(t *testing.T) {
	const (
	...
	logger := zerolog.Nop()
	replaySnapshotted := make(chan struct{})
	releaseReplay := make(chan struct{})
	handoffProgressed := make(chan struct{})
	var replayOnce sync.Once
	var progressOnce sync.Once
	replacementLateSends := atomic.Int32{}

	oldClient := &mockSubscribeClient{
		sendFn: func(req *dispatchercontracts.SubscribeToWorkflowRunsRequest) error {
			if req.WorkflowRunId == lateRunID {
				progressOnce.Do(func() {
					close(handoffProgressed)
				})
			}
			return nil
		},
	...
	replacementClient := &mockSubscribeClient{
		sendFn: func(req *dispatchercontracts.SubscribeToWorkflowRunsRequest) error {
			switch req.WorkflowRunId {
			case existingRunID:
				replayOnce.Do(func() {
					close(replaySnapshotted)
				})
				<-releaseReplay
			case lateRunID:
				replacementLateSends.Add(1)
			}
			return nil
		},
	}
	...
	require.True(t, listener.gate.tryStart(false))
	defer listener.gate.stop()

	listener.reg.store(existingRunID, "existing-session", func(event WorkflowRunEvent) error {
		return nil
	}, nil)
	listener.stream.onSendMuContention = func() {
		progressOnce.Do(func() {
			close(handoffProgressed)
		})
	}
	...
	go func() {
		reconnectErr <- listener.stream.connectOnce(context.Background())
	}()
	...
	go func() {
		addErr <- listener.AddWorkflowRun(lateRunID, "late-session", func(event WorkflowRunEvent) error {
			return nil
		})
	}()
	...
	assert.GreaterOrEqual(t, replacementLateSends.Load(), int32(1))
	require.NoError(t, listener.Close())
}
