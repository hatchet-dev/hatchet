...
func generateJWTKeysets(masterKey tink.AEAD) (privateEc256 []byte, publicEc256 []byte, err error) {
	...
	// TODO-CONTROL-PLANE: need to generate the public keyset for use in the API server
	// here's some code for that.
	//
	// print the public keyset for debugging purposes
	// pubBytes, err := insecureBytesFromHandle(publicHandle)

	// if err != nil {
	// 	err = fmt.Errorf("failed to get bytes from public handle: %w", err)
	// 	return
	// }

	// fmt.Println("PUBLIC KEYSET (base64-encoded JSON):", string(pubBytes))
