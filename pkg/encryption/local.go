...
func NewLocalEncryption(masterKey []byte, privateEc256 []byte, publicEc256 []byte) (*localEncryptionService, error) {
	a, envelope, err := newLocalEnvelope(masterKey)
...
// NewLocalDataEncryption creates a local encryption service for data encryption only, from the
// master keyset alone. It shares the envelope construction with NewLocalEncryption, so ciphertext
// written by either is readable by the other. Processes that never issue or verify JWTs (the
// out-of-process serverless operator) use this so they need not be given the JWT keysets;
// GetPrivateJWTHandle and GetPublicJWTHandle return nil.
func NewLocalDataEncryption(masterKey []byte) (EncryptionService, error) {
	_, envelope, err := newLocalEnvelope(masterKey)
...
// newLocalEnvelope builds the master AEAD from the cleartext master keyset and the envelope AEAD
// that encrypts data with per-record keys wrapped by it.
func newLocalEnvelope(masterKey []byte) (tink.AEAD, *aead.KMSEnvelopeAEAD, error) {
