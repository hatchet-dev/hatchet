...
async def test_cancel_queued_except_newest_keeps_only_the_newest_queued_run(
    hatchet: Hatchet,
) -> None:
    ...
    occupying_run = await concurrency_cancel_queued_except_newest_workflow.aio_run(
    ...
    for i in range(1, 11):
        ref = await concurrency_cancel_queued_except_newest_workflow.aio_run(
            WorkflowInput(group=group),
    ...
    occupying, *queued = runs
    *superseded, newest_queued = queued

    assert occupying.status == V1TaskStatus.COMPLETED

    assert (newest_queued.additional_metadata or {}).get("i") == "10"
    assert newest_queued.status == V1TaskStatus.COMPLETED
    assert all(r.status == V1TaskStatus.CANCELLED for r in superseded)

    ## the newest queued run should be pulled off the queue as soon as the occupying run frees
    ## the slot, not after some later polling cycle
    assert occupying.finished_at is not None
    assert newest_queued.started_at is not None
    assert newest_queued.started_at - occupying.finished_at < timedelta(seconds=3)
