...
def poll_for_trace(hatchet: Hatchet, run_id: str, min_spans: int = 1) -> list[OtelSpan]:
    # avoid race conditions between engine and sdk-side spans
    time.sleep(5)
...
async def test_otel_spans_created_on_task_run(hatchet: Hatchet) -> None:
    ...
    assert (
        len(spans) == 5
    ), "five spans: hatchet.engine.workflow_run, hatchet.engine.queued, hatchet.start_step_run, hatchet.engine.start_step_run, custom.child.span"
...
async def test_otel_spans_on_event_triggered_run(hatchet: Hatchet) -> None:
    ...
    event_spans = [s for s in spans if s.span_name == "hatchet.engine.event"]

    assert len(event_spans) == 1

    event_span = event_spans[0]

    assert event_span.span_attributes
    assert event_span.span_attributes.get(
        "hatchet.event_key"
...
async def test_otel_spans_on_child_spawn(hatchet: Hatchet) -> None:
    ...
    assert len(spans) == 11, """
        11 spans:
            ...
            - hatchet.run_workflow (SDK spawn call)
