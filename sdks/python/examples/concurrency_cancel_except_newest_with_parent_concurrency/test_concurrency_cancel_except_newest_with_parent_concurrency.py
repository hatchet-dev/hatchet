...
async def test_cancel_except_newest_keeps_newest_with_parent_concurrency(
    hatchet: Hatchet,
) -> None:
    """Task-level CANCEL_EXCEPT_NEWEST must still keep only the newest queued run even when
    ...
    workflow = concurrency_cancel_except_newest_with_parent_concurrency_workflow
    ...
    *superseded, newest_queued = queued_refs

    newest_details = await hatchet.runs.aio_get_details(newest_queued.workflow_run_id)
    assert newest_details.status == RunStatus.COMPLETED
