...
async def durable_event_task_with_filter(input: EmptyModel, ctx: DurableContext) -> None:
    # ❓ Durable Event With Filter
    res = await ctx.aio_wait_for(
        "event",
        UserEventCondition(event_key="user:update", expression="input.user_id == '1234'"),
    )
    # !!
...
def main() -> None:
    worker = hatchet.worker(
        "durable-event-worker", workflows=[durable_event_task, durable_event_task_with_filter]
    )
    worker.start()
