...
def patch_rest_transport_exceptions(content: str) -> str:
    ...
    """
    # Check if already patched with http_method support
    if "self.http_method" in content:
        return content
...
def patch_rest_error_diagnostics(content: str) -> str:
    ...
    Each typed exception includes http_method=method for retry logic.
    """
    ...
    # Check if already using typed exceptions WITH http_method
    if "http_method=method" in content:
        return content

    # Pattern for typed exceptions WITHOUT http_method (needs upgrade)
    ...
    replacement = (
        ...
        r"\1        ]\n"
        r"\1    )\n"
        r"\1    raise RestTLSError(status=0, reason=msg, http_method=method) from e\n"
        r"\1except (\n"
        ...
        r"\1        ]\n"
        r"\1    )\n"
        r"\1    raise RestConnectionError(status=0, reason=msg, http_method=method) from e\n"
        r"\1except (\n"
        ...
        r"\1        ]\n"
        r"\1    )\n"
        r"\1    raise RestTimeoutError(status=0, reason=msg, http_method=method) from e\n"
        ...
        r"\1        ]\n"
        r"\1    )\n"
        r"\1    raise RestProtocolError(status=0, reason=msg, http_method=method) from e\n"
    )

    # Try pattern for typed exceptions without http_method (upgrade case)
