...
class DurableTask(Generic[TWorkflowInput, R]):
    def __init__(
        self,
        fn: (
        ...
        self.fn = fn
        self.is_async_function = is_durable_async_fn(fn)
        self.workflow = workflow
    ...
    def call(self, ctx: DurableContext) -> R:
        if self.is_async_function:
            raise TypeError(f"{self.name} is not a sync function. Use `acall` instead.")
        ...
        if is_durable_sync_fn(sync_fn):
            return sync_fn(workflow_input, ctx)

        raise TypeError(f"{self.name} is not a sync function. Use `acall` instead.")

    async def aio_call(self, ctx: DurableContext) -> R:
        if not self.is_async_function:
            raise TypeError(
                f"{self.name} is not an async function. Use `call` instead."
            )
        ...
        if is_durable_async_fn(async_fn):
            return await async_fn(workflow_input, ctx)
