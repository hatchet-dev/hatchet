...
# v1/serverless.proto imports the same file the same way; its TS bindings are generated by
# sdks/typescript-serverless/generate-proto.sh, which compiles it with the extra proto path.
PROTOS=""
for f in $IN_DIR/**/*.proto; do
  case "$f" in
    */v1/operator.proto) continue ;;
    */v1/serverless.proto) continue ;;
  esac
  PROTOS="$PROTOS $f"
done
