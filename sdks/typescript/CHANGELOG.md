...
- Added a package `exports` map, so `import { HatchetClient } from '@hatchet-dev/typescript-sdk/v1'` now resolves under native Node ESM instead of failing with `ERR_UNSUPPORTED_DIR_IMPORT` (fixes #3613; supersedes #3650 by @dhimanAbhi, whose `.` and `./v1` entries it folds in). The map lists `.`, `/v1`, `/edge`, `/core` and every directory that ships an `index.js` explicitly, and wildcard entries keep every existing deep import resolving with or without the `.js` suffix. `scripts/check-exports.mjs` (`pnpm check:exports`) resolves each documented path from an ESM and a CommonJS scratch module against the built package in CI.
...
- Added a Connect `Transport` seam in `clients/transport`: `createNodeTransport(config)` speaks the gRPC protocol over HTTP/2 through `@connectrpc/connect-node`, derives its TLS settings from `tls_config` (`none`, `tls`, `mtls`, `ca_file`, `cert_file`, `key_file`, `server_name`) and attaches the bearer token to every call. `HatchetClientOptions.transport` injects a transport into the client.

### Changed

- The admin, event and v1 admin clients issue their unary RPCs (`TriggerWorkflow`, `BulkTriggerWorkflow`, `PutWorkflow`, `PutRateLimit`, `ScheduleWorkflow`, `Push`, `BulkPush`, `PutLog`, `PutStreamEvent`) through the Connect transport. The wire protocol, the request and response types and the status-code classification are unchanged; the streaming clients, the worker and the run listeners stay on `nice-grpc`. Errors from these calls are `ConnectError`s: `code` is the same numeric gRPC status code, the server's message is in `rawMessage`, and `message` reads `[already_exists] ...` where it used to read `/WorkflowService/TriggerWorkflow ALREADY_EXISTS: ...`.
- `EventClient` is constructed as `new EventClient(config, api, transport?)`; the `nice-grpc` channel and factory parameters are gone. The legacy `AdminClient` keeps its positional parameters and takes an optional `transport` last.
