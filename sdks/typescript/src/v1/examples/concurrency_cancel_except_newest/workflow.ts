...
export const concurrencyCancelExceptNewestWorkflow = hatchet.workflow<
  WorkflowInput,
  WorkflowOutput
>({
...
const step1 = concurrencyCancelExceptNewestWorkflow.task({
  name: 'step1',
...
concurrencyCancelExceptNewestWorkflow.task({
  name: 'step2',
  parents: [step1],
  fn: async (_, ctx): Promise<EmptyTaskOutput> => {
    for (let i = 0; i < 50; i += 1) {
