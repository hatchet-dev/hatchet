...
describe('conditions-e2e', () => {
  ...
  xit('waits, receives events, and completes branches', async () => {
    ...
    const waitForEventRandom =
      result.waitForEvent.randomNumber ?? result.wait_for_event.random_number;
    const waitForSleepRandom =
      result.waitForSleep.randomNumber ?? result.wait_for_sleep.random_number;
    ...
    const branchRandom =
      left?.randomNumber ?? right?.randomNumber ?? left?.random_number ?? right?.random_number;
    ...
    expect(sum).toBeGreaterThanOrEqual(
      startRandom + waitForEventRandom + waitForSleepRandom + branchRandom
    );
  }, 120_000);
});
