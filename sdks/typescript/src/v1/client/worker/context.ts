...
export class Context<T, K = {}> {
  ...
  log(message: string, level?: LogLevel, extra?: LogExtra) {
    ...
    const contextExtra = {
      ...
      taskRunId: this.action.taskRunId,
    ...
    promises.push(
      this.v1.event.putLog(taskRunId, message, level as any, this.retryCount(), extra?.extra)
    );
  ...
  async refreshTimeout(incrementBy: Duration) {
    ...
    await this.v1._v0.dispatcher.refreshTimeout(incrementBy, taskRunId);
  }
  ...
  async putStream(data: string | Uint8Array) {
    ...
    await this.v1._v0.event.putStream(taskRunId, data, index);
  }
