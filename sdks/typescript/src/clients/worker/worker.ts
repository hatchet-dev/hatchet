...
export class V0Worker {
  ...
  async handleStartStepRun(action: Action) {
    ...
    try {
      ...
      const success = async (result: any) => {
        this.logger.info(`Task run ${action.taskExternalId} succeeded`);
      ...
      const failure = async (error: any) => {
        this.logger.error(`Task run ${action.taskExternalId} failed: ${error.message}`);
      ...
      try {
        ...
        if (message.includes('Cancelled')) {
          this.logger.debug(`Task run ${action.taskExternalId} was cancelled`);
        } else {
          this.logger.error(
            `Could not wait for task run ${action.taskExternalId} to finish. ` +
  ...
  async handleCancelStepRun(action: Action) {
    ...
    try {
      this.logger.info(`Cancelling task run ${action.taskExternalId}`);
      ...
      if (future) {
        future.promise.catch(() => {
          this.logger.info(`Cancelled task run ${action.taskExternalId}`);
        });
