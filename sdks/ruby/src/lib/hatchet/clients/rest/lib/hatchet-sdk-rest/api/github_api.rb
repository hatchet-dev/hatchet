...
module HatchetSdkRest
  class GithubApi
    ...
    def sns_update_with_http_info(tenant, event, opts = {})
      ...
      if @api_client.config.client_side_validation && tenant.to_s.length < 36
        fail ArgumentError, 'invalid value for "tenant" when calling GithubApi.sns_update, the character length must be great than or equal to 36.'
      end
      ...
      if @api_client.config.client_side_validation && event.to_s.length < 1
        fail ArgumentError, 'invalid value for "event" when calling GithubApi.sns_update, the character length must be great than or equal to 1.'
      end
