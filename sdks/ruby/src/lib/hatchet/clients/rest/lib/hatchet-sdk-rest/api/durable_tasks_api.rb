...
module HatchetSdkRest
  class DurableTasksApi
    ...
    def v1_durable_task_event_log_list_with_http_info(tenant, durable_task, opts = {})
      ...
      if @api_client.config.client_side_validation && tenant.to_s.length < 36
        fail ArgumentError, 'invalid value for "tenant" when calling DurableTasksApi.v1_durable_task_event_log_list, the character length must be great than or equal to 36.'
      end
      ...
      if @api_client.config.client_side_validation && durable_task.to_s.length < 36
        fail ArgumentError, 'invalid value for "durable_task" when calling DurableTasksApi.v1_durable_task_event_log_list, the character length must be great than or equal to 36.'
      end
