  ...
  class V1LogLine
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `HatchetSdkRest::V1LogLine` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `HatchetSdkRest::V1LogLine`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@task_external_id.nil? && @task_external_id.to_s.length < 36
        invalid_properties.push('invalid value for "task_external_id", the character length must be great than or equal to 36.')
      end
    ...
    def task_external_id=(task_external_id)
      ...
      if task_external_id.to_s.length < 36
        fail ArgumentError, 'invalid value for "task_external_id", the character length must be great than or equal to 36.'
      end
