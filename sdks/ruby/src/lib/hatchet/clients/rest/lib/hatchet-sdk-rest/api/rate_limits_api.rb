...
module HatchetSdkRest
  class RateLimitsApi
    ...
    def rate_limit_delete_with_http_info(tenant, key, opts = {})
      ...
      if @api_client.config.client_side_validation && tenant.to_s.length < 36
        fail ArgumentError, 'invalid value for "tenant" when calling RateLimitsApi.rate_limit_delete, the character length must be great than or equal to 36.'
      end
    ...
    def rate_limit_list_with_http_info(tenant, opts = {})
      ...
      if @api_client.config.client_side_validation && tenant.to_s.length < 36
        fail ArgumentError, 'invalid value for "tenant" when calling RateLimitsApi.rate_limit_list, the character length must be great than or equal to 36.'
      end
