...
module HatchetSdkRest
  class SlackApi
    ...
    def slack_webhook_delete_with_http_info(slack, opts = {})
      ...
      if @api_client.config.client_side_validation && slack.to_s.length < 36
        fail ArgumentError, 'invalid value for "slack" when calling SlackApi.slack_webhook_delete, the character length must be great than or equal to 36.'
      end
    ...
    def slack_webhook_list_with_http_info(tenant, opts = {})
      ...
      if @api_client.config.client_side_validation && tenant.to_s.length < 36
        fail ArgumentError, 'invalid value for "tenant" when calling SlackApi.slack_webhook_list, the character length must be great than or equal to 36.'
      end
