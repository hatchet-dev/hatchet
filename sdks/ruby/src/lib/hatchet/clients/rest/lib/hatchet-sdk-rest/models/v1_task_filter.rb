...
module HatchetSdkRest
  class V1TaskFilter < ApiModelBase
    attr_accessor :since
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `HatchetSdkRest::V1TaskFilter` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `HatchetSdkRest::V1TaskFilter`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    # @param [Object] since Value to be assigned
    def since=(since)
      if since.nil?
        fail ArgumentError, 'since cannot be nil'
      end
    ...
    def hash
      [since, _until, statuses, workflow_ids, additional_metadata].hash
    end
