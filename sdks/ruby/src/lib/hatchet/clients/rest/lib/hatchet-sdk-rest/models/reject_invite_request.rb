  ...
  class RejectInviteRequest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `HatchetSdkRest::RejectInviteRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `HatchetSdkRest::RejectInviteRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @invite.to_s.length < 36
        invalid_properties.push('invalid value for "invite", the character length must be great than or equal to 36.')
      end
    ...
    def invite=(invite)
      ...
      if invite.to_s.length < 36
        fail ArgumentError, 'invalid value for "invite", the character length must be great than or equal to 36.'
      end
