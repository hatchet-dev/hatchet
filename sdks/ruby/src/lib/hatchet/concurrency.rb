...
module Hatchet
  ...
  module ConcurrencyLimitStrategy
    ...
    QUEUE = :queue
    CANCEL_EXCEPT_NEWEST = :cancel_except_newest
    CANCEL_EXCEPT_OLDEST = :cancel_except_oldest
  end
  ...
  class ConcurrencyExpression
    ...
    # @return [Symbol] Strategy when limit is exceeded (:cancel_in_progress, :cancel_newest, :group_round_robin, :queue, :cancel_except_newest, :cancel_except_oldest)
