...
type OperatorServiceClient interface {
	...
	//  2. The server sends only AssignedAction messages; the dispatcher fan-out owns the send
	//     side of the stream from the moment the worker is activated.
	...
	//  5. When the stream ends (client close, network failure, or engine shutdown) the worker is
...
type OperatorService_ListenClient interface {
	...
	Recv() (*contracts.AssignedAction, error)
	grpc.ClientStream
}
...
func (x *operatorServiceListenClient) Recv() (*contracts.AssignedAction, error) {
	m := new(contracts.AssignedAction)
...
type OperatorService_ListenServer interface {
	Send(*contracts.AssignedAction) error
...
func (x *operatorServiceListenServer) Send(m *contracts.AssignedAction) error {
