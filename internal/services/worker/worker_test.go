...
func TestFindAndReplace(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			input:       `context.playground("test", "Original override")`,
			value:       "test",
			override:    `"New override"`,
			expected:    `context.playground("test", "New override")`,
			expectError: false,
		},
		{
			name:        "Multiple Replacement",
			input:       `context.playground("test", "Original override"); context.playground("test", "Another original")`,
			value:       "test",
			override:    `"New override"`,
			expected:    `context.playground("test", "New override"); context.playground("test", "New override")`,
			expectError: false,
		},
		{
			name:        "Replace number",
			input:       `context.playground("test", 1234)`,
			value:       "test",
			override:    "5678",
			expected:    `context.playground("test", 5678)`,
			expectError: false,
		},
		{
			name:        "No Match",
			input:       `context.playground("no_match", "Original override")`,
			value:       "test",
			override:    `"New override"`,
			expected:    `context.playground("no_match", "Original override")`,
			expectError: false,
		},
		{
