...
func (ec *JobsControllerImpl) handleStepRunRequeue(ctx context.Context, task *taskqueue.Task) error {
	...
	for _, stepRun := range stepRuns {
		...
		g.Go(func() error {
			...
			isTimedOut := (scheduleTimeoutAt.IsZero() &&
				time.Now().After(stepRunCp.CreatedAt.Time.Add(defaults.DefaultScheduleTimeout))) ||
				scheduleTimeoutAt.Before(now)
			...
			stepRun, _, err := ec.repo.StepRun().UpdateStepRun(payload.TenantId, stepRunId, &repository.UpdateStepRunOpts{
