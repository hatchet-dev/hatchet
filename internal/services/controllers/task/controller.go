...
func New(fs ...TasksControllerOpt) (*TasksControllerImpl, error) {
	...
	t.timeoutBatchedQueueItemOperations = operation.NewTenantOperationPool(opts.p, opts.l, "timeout-batched-queue-items", timeout, "timeout batched queue items", t.processBatchedQueueItemTimeouts, operation.WithPoolInterval(
		opts.repov1.IntervalSettings(),
		jitter,
		1*time.Second,
		30*time.Second,
