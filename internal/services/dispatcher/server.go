...
func (s *DispatcherImpl) Listen(request *contracts.WorkerListenRequest, stream contracts.Dispatcher_ListenServer) error {
	...
	s.workers.Add(workerId, sessionId, newSubscribedWorker(stream, fin, workerId, s.defaultMaxWorkerLockAcquisitionTime, s.repov1.OLAP()))
...
func (d *DispatcherImpl) flushRefreshTimeout(ctx context.Context, key refreshTimeoutKey) (time.Time, error) {
	...
	for {
		...
		err = d.repov1.OLAPOutbox().MonitoringEvents(ctx, key.tenantId, v1.CreateMonitoringEventPayload{
