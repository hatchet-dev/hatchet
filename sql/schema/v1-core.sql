...
-- v1_task_evicted stores evicted task runs which are still logically RUNNING but no longer occupy worker slots.
CREATE TABLE v1_task_evicted (
    task_id bigint NOT NULL,
    task_inserted_at TIMESTAMPTZ NOT NULL,
    retry_count INTEGER NOT NULL,
    worker_id UUID,
...
CREATE INDEX v1_task_evicted_tenantId_timeoutAt_idx ON v1_task_evicted (tenant_id ASC, timeout_at ASC);

CREATE INDEX v1_task_evicted_tenantId_workerId_idx ON v1_task_evicted (tenant_id ASC, worker_id ASC) WHERE worker_id IS NOT NULL;
