...
CREATE TABLE v1_durable_event_log_entry (
    ...
    -- Set once the entry's trigger side effect (spawning a child RUN / registering WAIT_FOR match
    -- conditions) has committed, in a separate transaction from the entry's creation. A null value
    -- marks an entry whose trigger has not yet run -- either still in flight, or orphaned by a crash
    -- between the two transactions and awaiting re-trigger on replay. Always null for MEMO entries,
    -- which have no trigger side effect.
