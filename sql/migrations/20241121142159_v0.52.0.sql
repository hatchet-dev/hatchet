...
BEGIN
    ...
    IF EXISTS (
        ...
        WHERE table_name = 'StepRun_new'
    ) THEN
        RAISE NOTICE 'StepRun_new Present!';
    ELSE
        RAISE NOTICE 'StepRun_new Not Present! Creating StepRun_new';
            ...
            ALTER INDEX IF EXISTS "StepRun_jobRunId_status_tenantId_idx" RENAME TO "StepRun_jobRunId_status_tenantId_idx_old";
            ...
            CREATE INDEX  "StepRun_id_status" ON "StepRun" ("id", "status");
   ...
   RAISE NOTICE 'StepRun_new table created successfully now we try and swap the tables';
    ...
    WHILE retry_count < max_retries LOOP
        BEGIN 
                ...
                RAISE NOTICE 'Locking tables';
                LOCK TABLE "StepRun" IN ACCESS EXCLUSIVE MODE;
                -- LOCK TABLE "StepRun_new" IN ACCESS EXCLUSIVE MODE;


           RAISE NOTICE 'Locked tables';
        ...
        DROP INDEX IF EXISTS "StepRun_updatedAt_idx";
...
EXCEPTION
    WHEN OTHERS THEN
        ...
        ELSE
            -- Exit the loop if max retries are reached
            RAISE EXCEPTION 'Migration failed after % retries', retry_count;
        END IF;
...
BEGIN
                ...
                RAISE NOTICE 'Renamed tables now we try and create the triggers';
                        ...
                        RAISE NOTICE 'Modify Workflow Run table while we have it locked';
...
END;
-- 1. Create a new "Event" table with the desired schema (including the new identityId column)
-- atlas:txmode none
