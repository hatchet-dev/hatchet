...
var serverCmd = &cobra.Command{
  ...
  hatchet server --dashboard-port 9000

  # Start server with custom ports and project name
  hatchet server --dashboard-port 9000 --grpc-port 8077 --project-name my-hatchet

  # Start server with custom profile name
  hatchet server --profile my-local`,
...
func init() {
	...
	serverCmd.Flags().IntP("dashboard-port", "d", 0, "Port for the Hatchet dashboard (default: auto-detect starting at 8888)")
	serverCmd.Flags().IntP("grpc-port", "g", 0, "Port for the Hatchet gRPC server (default: auto-detect starting at 7077)")
	serverCmd.Flags().StringP("project-name", "p", "", "Docker project name for containers (default: hatchet-cli)")
	serverCmd.Flags().StringP("profile", "n", "local", "Name for the local profile (default: local)")
}
