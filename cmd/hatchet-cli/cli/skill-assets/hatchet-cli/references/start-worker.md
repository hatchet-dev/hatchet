...
These steps assume a profile that points at a Hatchet deployment. For local development the worker can instead run against embedded mode, with no token or profile; see `local-dev-embedded.md`.
