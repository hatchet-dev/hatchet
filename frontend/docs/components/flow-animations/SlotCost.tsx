...
const WEENIE_TRACKS = CELL_SCHEDULE.flatMap(({ col, row, runs }) =>
  runs.flatMap(([dock, end], i) =>
    loopTracks(
      `sc-w${col}${row}-${i}`,
      weenieKeyframes(dock, end, COL_X[col], ROW_Y[row]),
    ),
  ),
);
...
const omegaKeyframes = ({
  col,
  dock,
  end,
  waitFrom,
}: OmegaSpec): FlowKeyframe[] => {
  ...
  const keyframes: FlowKeyframe[] = [
    ...
    },
    {
      ...
      y: OMEGA_Y,
      ease: "linear",
      state: waitFrom ? "waiting" : "inflight",
    },
  ];
...
const OMEGA_TRACKS = OMEGAS.flatMap((spec) =>
  loopTracks(spec.id, omegaKeyframes(spec)),
);
...
export const SlotCost = ({ style }: { style?: CSSProperties }) => (
