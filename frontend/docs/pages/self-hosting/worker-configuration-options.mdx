...
| `slots`         | Maximum number of concurrent runs (mapped to `slot_config.default`)          | `100`         |
| `durable_slots` | Maximum number of concurrent durable tasks (mapped to `slot_config.durable`) | `1000`        |
| `slot_config`   | Slot capacities for the worker (slot_type -> units)                          |               |
