...
| `slots`                  | Maximum number of concurrent runs (mapped to `slot_capacities.default`)          | `100`         |
| `durable_slots`          | Maximum number of concurrent durable tasks (mapped to `slot_capacities.durable`) | `1000`        |
| `slot_capacities`        | Slot capacities for the worker (slot_type -> units)                              |               |
