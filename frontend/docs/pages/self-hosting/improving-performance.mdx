...
Hatchet batches message-queue writes to cut down on round-trips to the broker and database. Both the publish (outgoing) buffer and the subscribe (incoming) buffer are governed by the same three settings, applied per `(tenant, message)` buffer:
