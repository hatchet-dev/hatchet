...
The dispatcher batches events internally to reduce the total number of database queries, which improves throughput at the cost of a small increase in latency. Two environment variables control this batching:

```sh
# Number of workflow run events the dispatcher batches before flushing (default: 1000)
SERVER_WORKFLOW_RUN_BUFFER_SIZE=1000

# How long the stream event buffer waits for out-of-order events before flushing (default: 5s)
SERVER_STREAM_EVENT_BUFFER_TIMEOUT=5s
```

Increasing `SERVER_WORKFLOW_RUN_BUFFER_SIZE` batches more work per write, which lowers database load under high throughput. Benchmarking and tuning on your own infrastructure is recommended to find the optimal values for your workload and use case.
