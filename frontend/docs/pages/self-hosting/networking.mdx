...
The base path is injected into the served `index.html` at request time, so a single image works for any subpath without rebuilding the frontend.
...
| Combined `dashboard` image        | `BASE_PATH`               | Substituted into `index.html` by the container entrypoint.  |
...
Two routing rules are required on the reverse proxy sitting in front of Hatchet:

1. **Strip the subpath prefix before forwarding to the frontend.** The static file server serves assets and SPA routes from its own root, so the proxy must remove `/hatchet` from the request path (Caddy's `handle_path`, or the nginx ingress `rewrite-target` annotation).
...
example.com {
    ...
    # handle_path strips the /hatchet prefix before proxying to the frontend
    handle_path /hatchet/* {
...
With `frontend.env.BASE_PATH` set to `/hatchet/` and the proxy above, the dashboard is reachable at `https://example.com/hatchet/` and API calls continue to hit `https://example.com/api/...`.
