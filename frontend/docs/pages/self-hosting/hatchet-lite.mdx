...
### Accessing Hatchet Lite and Generating an API Token
...
You can generate an API token by logging in and navigating to the "General" settings page, clicking on the "API Tokens" tab, and then clicking "Create API Token".

Alternatively, you can generate an API token from your terminal via:

```
export HATCHET_CLIENT_TOKEN="$(docker compose -f docker-compose.hatchet.yml exec hatchet-lite /hatchet-admin token create --config /config --tenant-id 707d0855-80ab-4e1f-a156-f1c4546cbf52 | xargs)"
echo "generated token: $HATCHET_CLIENT_TOKEN"
```

### Generate a `.env` file

The examples below read from a `.env` file in the root of your project. This file should contain the following:

```
HATCHET_CLIENT_TOKEN=your-generated-token
