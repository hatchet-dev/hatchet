...
The quickstart job bootstraps Hatchet's encryption and auth secrets into a Kubernetes Secret (`hatchet-config`). It lives in the `hatchet-api` subchart, so in the `hatchet-stack` and `hatchet-ha` charts it is configured under the `api` alias and is enabled by default:
...
The worker token job generates a client token after the engine is running. Like the quickstart job, it lives in the `hatchet-api` subchart:
...
The `sharedConfig` object also allows you to seed the database with a default tenant and user. The default admin credentials are set directly on `sharedConfig`, and the remaining seed values are passed as environment variables via `sharedConfig.env`:
