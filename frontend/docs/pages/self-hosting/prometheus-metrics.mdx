...
| `hatchet_tenant_queue_size`                                  | Gauge     | The current number of queued items, by queue and workflow name                                         |
| `hatchet_tenant_additional_metadata_queue_size`              | Gauge     | The current number of queued items, by queue and additional metadata key-value pair                    |
...
The `hatchet_tenant_queued_to_assigned_by_workflow` counter and the `hatchet_tenant_queued_to_assigned_time_seconds_by_workflow` histogram are per-workflow variants of the queued-to-assigned metrics, carrying a `workflow_name` label so queue latency can be attributed to individual workflows.

The queue size gauges are polled from the database every 15 seconds, so they reflect the backlog with up to that much delay. `hatchet_tenant_queue_size` reports the number of queued items per `queue` and `workflow_name`, and is safe to sum. Items queued behind a concurrency strategy are not counted until they enter the queue.

`hatchet_tenant_additional_metadata_queue_size` reports queued items per queue, grouped by additional metadata `(key, value)` pairs, using the `queue`, `key`, and `value` Prometheus labels; only scalar metadata values (strings, numbers, booleans) are reported. Summing across `queue` values is safe (an item is in exactly one queue), but like the worker label slot metrics, an item counts towards every metadata key it carries, so do not sum this gauge across different `key` values.

<Callout type="warning">
  Every distinct additional metadata `(key, value)` pair on a queued task
  creates its own Prometheus series. Avoid high-cardinality metadata values
  (e.g. per-run identifiers) on workflows you run at volume, or your metrics
  storage will pay for it.
</Callout>

When a queue drains, its series report `0` on the next poll and are removed shortly after, so autoscalers observe the drop to zero. Guard queries against the series being absent (e.g. `sum(...) or vector(0)`) if you scale to zero.
...
```promql
sum(hatchet_tenant_additional_metadata_queue_size{key="product", value="search"}) or vector(0)
```
