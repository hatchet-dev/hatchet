...
`hatchet_tenant_additional_metadata_queue_size` reports queued items grouped by additional metadata `(key, value)` pairs, using the `key` and `value` Prometheus labels; only scalar metadata values (strings, numbers, booleans) are reported. Like the worker label slot metrics, an item counts towards every metadata key it carries, so do not sum this gauge across different `key` values.
