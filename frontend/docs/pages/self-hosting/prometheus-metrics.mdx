...
| `hatchet_tenant_additional_metadata_queue_size`              | Gauge     | The current number of queued items, by queue and additional metadata key-value pair (scalar values only). Polled from the database every 15 seconds; an item counts towards every metadata key it carries, so do not sum across `key` values |
...
```promql
sum(hatchet_tenant_additional_metadata_queue_size{key="pool", value="gpu"}) or vector(0)
```
