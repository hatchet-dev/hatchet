...
| `hatchet_tenant_additional_metadata_queue_size`  | Gauge     | The current number of queued items, by additional metadata key-value pair            |
...
The queue size gauges are polled from the database every 15 seconds, so they reflect the backlog with up to that much delay. `hatchet_tenant_queue_size` reports the number of queued items per `queue` and `workflow_name`, and is safe to sum. `hatchet_tenant_additional_metadata_queue_size` reports the number of queued items carrying each additional metadata `(key, value)` pair, using the `key` and `value` Prometheus labels; only scalar metadata values (strings, numbers, booleans) are reported. Like the worker label slot metrics, an item counts towards every metadata key it carries, so do not sum this gauge across different `key` values. Items queued behind a concurrency strategy are not counted until they enter the queue.
