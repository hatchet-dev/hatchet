...
| `SERVER_AUTH_AZURE_ENABLED`            | Whether Azure auth is enabled                               | `false`                          |
| `SERVER_AUTH_AZURE_CLIENT_ID` ⚠️       | Azure auth client ID (required if Azure auth enabled)       |                                  |
| `SERVER_AUTH_AZURE_CLIENT_SECRET` ⚠️   | Azure auth client secret (required if Azure auth enabled)   |                                  |
| `SERVER_AUTH_AZURE_TENANT_ID`          | Azure AD tenant ID/domain (empty allows any organization)   | `organizations`                  |
| `SERVER_AUTH_AZURE_SCOPES`             | Azure auth scopes                                           | `["openid", "profile", "email"]` |
