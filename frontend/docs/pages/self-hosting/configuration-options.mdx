...
| `SERVER_MSGQUEUE_PUBSUB_KIND`                       | Pub/sub kind for best-effort notifications (`rabbitmq`, `postgres`, or `nats`; inherits `SERVER_MSGQUEUE_KIND` when unset)                                                                                                                           |                  |
...
| `SERVER_MSGQUEUE_PUBSUB_NATS_URL`                   | Pub/sub NATS seed URL(s), comma-separated for a cluster (core NATS only, best-effort at-most-once). Prefer bare hosts and set username/password separately so rediscovered peers authenticate. Server must set `max_payload` to at least `16777216`. |                  |
| `SERVER_MSGQUEUE_PUBSUB_NATS_USERNAME`              | Pub/sub NATS username (passed as connect options; recommended for clusters)                                                                                                                                                                          |                  |
| `SERVER_MSGQUEUE_PUBSUB_NATS_PASSWORD`              | Pub/sub NATS password (passed as connect options; recommended for clusters)                                                                                                                                                                          |                  |
| `SERVER_MSGQUEUE_PUBSUB_NATS_SUBJECT_PREFIX`        | Pub/sub NATS subject prefix (prepended to topic names, joined with `.`)                                                                                                                                                                              | `hatchet.pubsub` |
