...
| `K8S_POD_NAMESPACE`             | Prefix added to the Postgres `application_name` (e.g. `<namespace>:<service>`) so connections can be attributed in `pg_stat_activity`; typically populated from the pod namespace via the downward API |                     |
...
Controls how task payloads are stored and the migration ("dual writes") between the inline and external payload stores.

| Variable                                                        | Description                                               | Default Value |
| --------------------------------------------------------------- | --------------------------------------------------------- | ------------- |
| `SERVER_PAYLOAD_STORE_ENABLE_PAYLOAD_DUAL_WRITES`               | Dual-write task payloads to the external store            | `true`        |
| `SERVER_PAYLOAD_STORE_ENABLE_TASK_EVENT_PAYLOAD_DUAL_WRITES`    | Dual-write task event payloads to the external store      | `true`        |
| `SERVER_PAYLOAD_STORE_ENABLE_DAG_DATA_PAYLOAD_DUAL_WRITES`      | Dual-write DAG data payloads to the external store        | `true`        |
| `SERVER_PAYLOAD_STORE_ENABLE_OLAP_PAYLOAD_DUAL_WRITES`          | Dual-write OLAP payloads to the external store            | `true`        |
