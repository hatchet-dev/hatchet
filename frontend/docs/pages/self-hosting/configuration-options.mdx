...
| `SERVER_MSGQUEUE_PUBSUB_NATS_URL`                   | Pub/sub NATS URL (comma-separated for a cluster; core NATS only, best-effort at-most-once). Server must set `max_payload: 16777216` (1MiB default is insufficient). Publishes fail while disconnected; polling covers gaps. |               |
