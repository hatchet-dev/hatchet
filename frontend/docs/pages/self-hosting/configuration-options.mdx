...
| `DATABASE_PGBOUNCER_URL`       | PgBouncer connection string; when set, the main pool uses this URL and `DATABASE_URL` is used for DDL operations |                     |
| `DATABASE_DDL_POOL_MAX_CONNS`  | Max connections for the DDL pool (used for DDL-modifying operations that cannot go through PgBouncer)            | `5`                 |
| `DATABASE_DDL_POOL_MIN_CONNS`  | Min connections for the DDL pool                                                                                 | `1`                 |
| `DATABASE_ENFORCE_UTC_TIMEZONE` | Enforce that the database timezone is UTC; server will panic on startup if not UTC                              | `true`              |
