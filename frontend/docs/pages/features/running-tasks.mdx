...
1. Tasks can be [run, and have their results waited on](/features/run-with-results)
2. Tasks can be [enqueued without waiting for their results ("fire and forget")](/features/run-no-wait).
3. Tasks can be run on [cron schedules](/features/cron-runs).
4. Tasks can be [triggered by events](/features/run-on-event).
5. Tasks can be [scheduled for a later time](/features/scheduled-runs).
