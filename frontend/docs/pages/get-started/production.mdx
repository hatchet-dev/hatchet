...
Workers are long-lived processes, so treat them like any other production service. Give them enough CPU and memory that a spike in traffic won't starve tasks or trigger OOM kills, and tune [slots](/concepts/workers#slots) based on what you actually observe rather than what seems reasonable on paper.
...
If workers are not appearing in the dashboard, tasks are failing to send, or phantom workers are showing up, see the [troubleshooting guide](/get-started/troubleshooting-workers).
