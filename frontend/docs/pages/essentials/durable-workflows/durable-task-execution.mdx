...
**Avoid side effects before checkpoints.** Non-deterministic operations (random numbers, current timestamps, external API calls) should be wrapped in `Memo` so their results are recorded and replayed consistently. See [Best Practices](/concepts/durable-best-practices) for more guidance.

<Callout type="warning">
  If your workflow can be expressed as a [DAG](/concepts/dags), prefer regular
...
<Cards>
  <Cards.Card title="Durable Sleep" href="/concepts/durable-sleep">
    Pause tasks for exact durations with crash-safe timing guarantees.
  </Cards.Card>
  <Cards.Card title="Durable Events" href="/concepts/durable-events">
