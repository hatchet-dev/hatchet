...
**Never change the order of durable operations.** If your task calls `SleepFor` then `WaitForEvent`, don't later swap their order. Existing checkpoints depend on the original sequence. Changing it breaks replay.

**Avoid side effects before checkpoints.** Non-deterministic operations (random numbers, current timestamps, external API calls) should be wrapped in `Memo` so their results are recorded and replayed consistently. See [Best Practices](/concepts/durable-workflows/durable-task-execution/durable-best-practices) for more guidance.

<Callout type="warning">
  If you know the shape of your work upfront  - a fixed set of tasks with declared dependencies  - prefer a [DAG](/concepts/durable-workflows/directed-acyclic-graphs).
  DAGs are inherently deterministic and don't require you to think about replay safety.
  Use durable tasks when the shape of work is dynamic: the number of children, the control flow, or the next step depends on runtime data.
</Callout>
...
<Cards>
  <Cards.Card title="Durable Sleep" href="/concepts/durable-workflows/durable-task-execution/durable-sleep">
    Pause tasks for exact durations with crash-safe timing guarantees.
  </Cards.Card>
  <Cards.Card title="Durable Events" href="/concepts/durable-workflows/durable-task-execution/durable-events">
...
Want to dive deeper? The [Durable Execution concepts page](/concepts/durable-workflows/durable-task-execution) covers how Hatchet runs durable tasks, slot management, eviction, and or-groups.
