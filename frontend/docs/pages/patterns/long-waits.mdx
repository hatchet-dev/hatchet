...
<Callout type="info">
  ...
  across worker restarts. See [Durable Sleep](/concepts/durable-sleep) and [Durable
  Events](/concepts/durable-events) for API details.
</Callout>
...
<Cards>
  <Cards.Card title="Delayed Notifications" href="/concepts/durable-sleep">
    Send a follow-up email or push notification hours or days after an initial
    action.
  </Cards.Card>
  <Cards.Card title="Human-in-the-Loop Approval" href="/concepts/durable-events">
    Pause a workflow until a human approves, rejects, or provides input via an
    external event.
  </Cards.Card>
  <Cards.Card title="Scheduled Follow-Ups" href="/concepts/durable-sleep">
    Implement drip campaigns, trial expiration checks, or periodic status polls
    with precise timing.
  </Cards.Card>
  <Cards.Card title="External System Callbacks" href="/concepts/durable-events">
...
- [Durable Sleep](/concepts/durable-sleep): pause for a fixed duration with exact timing guarantees
- [Durable Events](/concepts/durable-events): wait for external signals with optional CEL filters
- [Conditional Workflows](/concepts/conditional-workflows): combine long waits with branching logic
