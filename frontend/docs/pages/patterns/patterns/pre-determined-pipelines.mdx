...
A **pre-determined pipeline** is a workflow where the sequence of tasks and their dependencies are defined ahead of time as a Directed Acyclic Graph (DAG). Unlike [fanout](/patterns/patterns/fanout), where children are spawned dynamically at runtime, pipelines have a fixed structure that is known before execution begins.
...
- [Fanout](/patterns/patterns/fanout): dynamically spawn tasks at runtime instead
