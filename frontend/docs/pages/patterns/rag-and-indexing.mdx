...
| ------------------------------------------ | ------------------------------------------------------------------------------ |
| **[Child Spawning](/concepts/child-spawning)** | Fan out to one task per document/chunk; process thousands in parallel          |
...
| **[DAG Workflows](/concepts/dags)**            | Chain pipeline stages (ingest → chunk → embed → index) with clear dependencies |
...
<Callout type="warning">
  ...
  [Concurrency Control](/concepts/concurrency) to limit how many run simultaneously.
</Callout>
...
- [DAG Workflows](/concepts/dags): define multi-stage pipelines with task dependencies
- [Rate Limits](/concepts/rate-limits): configure rate limiting for embedding APIs
- [Child Spawning](/concepts/child-spawning): fan out to per-document tasks
