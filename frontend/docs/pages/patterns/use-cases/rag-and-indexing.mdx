...
| ------------------------------------------ | ------------------------------------------------------------------------------ |
| **[Child Spawning](/features/child-spawning)** | Fan out to one task per document/chunk; process thousands in parallel          |
| **[Rate Limits](/features/rate-limits)**       | Throttle embedding API calls across all workers to stay within provider limits |
| **[Retry Policies](/features/retry-policies)** | Retry failed chunks individually without re-processing the whole pipeline      |
| **[Concurrency](/features/concurrency)**       | Fair scheduling with GROUP_ROUND_ROBIN for multi-tenant indexing               |
| **[DAG Workflows](/features/dags)**            | Chain pipeline stages (ingest → chunk → embed → index) with clear dependencies |
| **[Event Triggers](/features/run-on-event)**      | Start indexing automatically when new documents arrive                         |
| **[Cron Triggers](/features/cron-runs)**          | Schedule periodic re-indexing or incremental updates                           |
| **[Bulk Run](/features/bulk-run)**                | Trigger indexing for thousands of documents in a single API call               |

<Callout type="info">
  RAG pipelines are a natural fit for [Pre-Determined
  Pipelines](/patterns/patterns/pre-determined-pipelines). The stages (ingest,
  chunk, embed, index) are fixed and map directly to a DAG. Use
  [Fanout](/patterns/patterns/fanout) within the chunking stage to parallelize.
</Callout>
...
<Callout type="warning">
  ...
  [Concurrency Control](/features/concurrency) to limit how many run simultaneously.
</Callout>
...
<Cards>
  <Cards.Card
    ...
    href="/patterns/patterns/pre-determined-pipelines"
  >
    The fixed-stage DAG pattern that RAG pipelines are built on.
  </Cards.Card>
  <Cards.Card title="Fanout" href="/patterns/patterns/fanout">
    Parallelize document and chunk processing across your worker fleet.
  </Cards.Card>
  <Cards.Card title="Cycles" href="/patterns/patterns/cycles">
    Implement incremental indexing that re-crawls until all changes are
    processed.
  </Cards.Card>
  <Cards.Card title="Batch Processing" href="/patterns/use-cases/batch-processing">
...
- [DAG Workflows](/features/dags): define multi-stage pipelines with task dependencies
- [Rate Limits](/features/rate-limits): configure rate limiting for embedding APIs
- [Child Spawning](/features/child-spawning): fan out to per-document tasks
- [Concurrency Control](/features/concurrency): fair scheduling for multi-tenant indexing
