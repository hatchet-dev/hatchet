...
| **[Autoscaling](/essentials/autoscaling-workers)**                                       | Scale workers up during batch processing, down when idle                |

<Callout type="info">
  The batch processing pattern is [Fanout](/patterns/fanout) applied at scale.
  For fixed multi-stage processing (e.g., validate → transform → load), combine
  with [Pre-Determined Pipelines](/patterns/pre-determined-pipelines).
</Callout>
...
<Cards>
  ...
  <Cards.Card title="RAG & Indexing" href="/patterns/rag-and-indexing">
