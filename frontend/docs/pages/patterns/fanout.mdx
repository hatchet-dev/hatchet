...
<Callout type="info">
  Fanout is built on top of [Procedural Child Spawning](/features/child-spawning).
...
<Callout type="warning">
  ...
  enough slots to handle the concurrent load. See [Worker Slots](/guide/workers)
...
<Cards>
  <Cards.Card title="Batch Processing" href="/features/bulk-run">
    Process thousands of items (images, documents, records) in parallel with
    automatic load distribution across workers.
  </Cards.Card>
  <Cards.Card title="Map-Reduce Pipelines" href="/features/dags">
    Fan out to map tasks, then reduce results in a downstream DAG step.
  </Cards.Card>
  <Cards.Card title="Multi-Provider AI Inference" href="/features/child-spawning">
    Send the same prompt to multiple LLM providers concurrently and pick the
    best response.
  </Cards.Card>
  <Cards.Card title="Notification Delivery" href="/features/rate-limits">
...
- [Procedural Child Spawning](/features/child-spawning): learn the full child task API
- [DAG Workflows](/features/dags): combine fanout with declarative task dependencies
- [Concurrency Control](/features/concurrency): limit how many children run simultaneously
- [Rate Limits](/features/rate-limits): throttle child task execution rates
