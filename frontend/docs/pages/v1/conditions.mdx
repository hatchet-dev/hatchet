...
DAG tasks can declare multiple conditions that work together to control when and whether a task runs. Conditions of different types (parent conditions, [event conditions](/v1/events), and [sleep conditions](/v1/durable-workflows/sleep)) can be mixed on a single task using **or groups**.
