...
<Callout type="info">
  For self-hosted engines, there may be additional gRPC configuration
  options needed. See the [Self-Hosting](/self-hosting/worker-configuration-options)
  docs for details.
</Callout>
...
<Callout type="info">
  ...
  bottlenecked by another resource. If the worker is CPU-bound, memory-bound,
  or waiting on network I/O, more slots will just increase contention.
</Callout>
