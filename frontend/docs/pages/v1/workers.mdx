...
A worker needs a name and a set of tasks or tasks (or workflows, more on this later) to register:
...
Every worker has a fixed number of **slots** that control how many tasks it can run concurrently, which can be configured with the `slots` option on the worker. For instance, if `slots` is set to 5, the worker will run up to five tasks at the concurrently at any time. Any additional tasks wait in the queue until a slot opens up.
