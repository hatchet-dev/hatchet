...
Call `run` with `wait_for_result=False` on a `Task` or `Workflow` object to enqueue it fire-and-forget. This returns a `WorkflowRunRef` you can use to access the run ID and result later. (The older `run_no_wait` method is deprecated in favor of `run(wait_for_result=False)`.)
