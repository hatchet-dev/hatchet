...
Start by declaring a policy in your task/workflow definition using your SDK's
durable-task eviction options.
...
Set `allowCapacityEviction` to `true` and optionally provide a TTL and priority
to let Hatchet evict waiting durable tasks when needed.
...
If you disable capacity eviction and do not set a TTL, the task remains pinned
to its worker slot while waiting.
