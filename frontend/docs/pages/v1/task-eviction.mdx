...
**Capacity-based eviction**: Whether Hatchet can evict this task when worker slots are under pressure. Set `allow_capacity_eviction=False` (Python), `allowCapacityEviction: false` (TypeScript), `AllowCapacityEviction: false` (Go), or `allow_capacity_eviction: false` (Ruby) for critical tasks that should not be interrupted. These tasks keep their slot until their wait completes normally.
...
<Callout type="info">
To prevent a task from ever being evicted, set `ttl` to `None` (Python), `undefined` (TypeScript), `0` (Go), or `nil` (Ruby) and disable capacity eviction. This is useful for tasks holding critical state that cannot be checkpointed, but the task will consume a slot for the entire wait duration.
</Callout>

### EvictionPolicy reference

<UniversalTabs items={["Python", "TypeScript", "Go", "Ruby"]} optionKey="sdk">
...
Pass an `eviction_policy` to `hatchet.durable_task()`:

| Parameter | Type | Default | Description |
| --- | --- | --- | --- |
| `ttl` | `Integer \| nil` | `900` (15 minutes) | Maximum wait duration in seconds before TTL-based eviction. Set to `nil` to disable. |
| `priority` | `Integer` | `0` | Lower values are evicted first under capacity pressure. |
| `allow_capacity_eviction` | `Boolean` | `true` | Whether the task can be evicted under slot pressure. |
