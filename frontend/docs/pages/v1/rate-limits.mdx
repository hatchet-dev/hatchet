...
# Rate Limiting Task Runs in Hatchet
...
To rate limit an entire workflow run, it's recommended to specify the rate limit configuration on the entry task (i.e., the first task in the workflow). This will gate the execution of all downstream tasks in the workflow.
...
1. Rate limiting individual users or tenants based on user roles or subscription tiers
2. Dynamically adjusting limits based on real-time system load or other factors
...
You can add one or more rate limits to a task by adding the `rate_limits` configuration to the task definition.
