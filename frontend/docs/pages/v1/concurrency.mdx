...
2. **Resource management**: If your tasks are resource-intensive (e.g., they make external API calls or perform heavy computations), running too many instances concurrently can overload your system. By limiting concurrency, you can ensure your system remains stable and responsive.
...
To set up concurrency on workers, specify a concurrency expression on the workflow or task. The `expression` field determines
the key which is used to evaluate the concurrency type, and the maximum runs limits the number of concurrent runs.
...
1. Determine the group that the instance belongs to based on the `expression` function defined in the task's concurrency configuration.
