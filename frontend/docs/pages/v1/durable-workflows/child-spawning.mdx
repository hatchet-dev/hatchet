...
[Concurrency](/v1/flow-control/concurrency) controls how many children run simultaneously. Hatchet distributes child tasks across available workers, so adding workers increases throughput without code changes. For rate-limited external services (OCR, LLM APIs), combine with [Rate Limits](/v1/flow-control/rate-limits) to throttle child execution across all workers.
