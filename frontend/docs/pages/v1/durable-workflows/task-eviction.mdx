...
<Callout type="info">
  ...
  example, to wait for an event based on intermediate results), use a
  [durable task](/v1/durable-workflows/durable-task-execution) instead. DAG
  tasks run from start to finish once scheduled.
</Callout>
