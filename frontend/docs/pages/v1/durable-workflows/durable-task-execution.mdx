...
<Callout type="info">
  ...
  before execution begins), use a [DAG](/v1/durable-workflows/directed-acyclic-graphs)
  instead. You can always [mix both patterns](/v1/durable-workflows/mixing-patterns)
...
<Callout type="info">
  The `durable_task` decorator gives the function a `DurableContext` instead of a
  regular `Context`. This is the only difference in declaration; the task
