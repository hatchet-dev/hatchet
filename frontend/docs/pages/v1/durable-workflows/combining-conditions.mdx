...
DAG tasks can declare multiple conditions that work together to control when and whether a task runs. Conditions of different types — [parent conditions](/v1/durable-workflows/parent-conditions), [event conditions](/v1/durable-workflows/event-conditions), and [sleep conditions](/v1/durable-workflows/sleep-conditions) — can be mixed on a single task using **or groups**.
...
<Callout type="info">
  Durable tasks have an analogous feature: [or groups in durable wait
  conditions](/v1/durable-workflows/durable-task-execution#or-groups). In
  a DAG, or groups are declared upfront on the task definition. In a durable
  task, they're passed to `WaitFor` at runtime.
</Callout>
