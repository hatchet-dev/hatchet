...
<Callout type="info">
  Run on event evaluates workflows to run at the time of the event. If an event is received before the workflow is registered, the workflow will not be run.
</Callout>
