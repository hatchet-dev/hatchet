...
<Callout type="warning">
  Durable tasks must only either call methods on the durable context or spawn children, and they must be deterministic given the event history. For example, you should _not_ directly access your database or an external API, or generate random numbers and use them for control flow inside of a durable task. Have your durable task spawn children to do this sort of work instead.
</Callout>
