...
<Callout type="info">
  The examples below use a placeholder scope like `foobarbaz` for simplicity. In
  a real application, a scope is typically an identifier for the entity the
  filter belongs to, such as a customer, user, or organization id.
</Callout>
