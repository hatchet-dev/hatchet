...
To scale on queue depth via Prometheus instead of the Task Stats API, Hatchet exposes queue size gauges, including one grouped by the additional metadata `(key, value)` pairs on queued tasks. If your GPU workloads tag their workflow runs with additional metadata (e.g. `pool: gpu`), the GPU backlog can drive its own scaler:

```promql
sum(hatchet_tenant_additional_metadata_queue_size{tenant_id="<tenant-id>", key="pool", value="gpu"}) or vector(0)
```
