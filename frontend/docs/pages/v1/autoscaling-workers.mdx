...
To scale on queue depth via Prometheus instead of the Task Stats API, Hatchet exposes queue size gauges, including one grouped by the additional metadata `(key, value)` pairs on queued tasks. If each of your products tags its workflow runs with additional metadata (e.g. `product: search`), each product's backlog can drive its own scaler:

```promql
sum(hatchet_tenant_additional_metadata_queue_size{tenant_id="<tenant-id>", key="product", value="search"}) or vector(0)
```
