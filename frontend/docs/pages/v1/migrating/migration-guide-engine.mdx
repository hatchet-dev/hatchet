...
<Callout type="warning">
  This guide covered upgrading from the legacy **v0 engine** to **v1**. The v0
  engine has since been removed — all tenants now run on v1 — so this page is
  kept for historical reference only and is no longer relevant to current
  deployments.
</Callout>
...
New tenants are created on the v1 engine by default, including on self-hosted instances — no additional configuration is required. To move an existing tenant, use the "Upgrade to v1" toggle described in [How to upgrade](#how-to-upgrade).
