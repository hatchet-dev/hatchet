...
<Callout type="warning">
  The runtime MCP endpoint must be deployed with `HATCHET_CLIENT_API_URL` set to
  your Hatchet API base URL (e.g. `https://api.hatchet.run`). For self-hosted
  Hatchet, point it to your instance. Write tools (`cancel_run`, `replay_run`)
  require `HATCHET_MCP_ALLOW_WRITES=true`.
</Callout>
