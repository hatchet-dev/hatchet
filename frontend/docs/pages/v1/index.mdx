...
- **[Tasks](/v1/your-first-task)** — the fundamental unit of work. A task wraps a single function and gives Hatchet everything it needs to schedule, execute, and observe it.
...
- **AI agents** — features like [webhooks](/v1/runnables/webhooks), [child spawning](/v1/durable-workflows/dag-child-spawning), and dynamic workflows are designed to support agentic patterns.
...
⚡️ **Low-Latency For Real-Time Workloads** - Sub-25ms task dispatch for hot workers with thousands of concurrent tasks. Smart assignment rules handle [rate-limits](/v1/flow-control/rate-limits), [fairness](/v1/flow-control/concurrency), and [priorities](/v1/flow-control/priority) without complex configuration.

🪨 **Durability for Long Running Jobs** - Every task invocation is durably logged to PostgreSQL. With [durable execution](/v1/durable-workflows/durable-task-execution), when jobs fail your workflow will resume exactly where you left off — no lost work, no duplicate LLM calls, no engineer headaches.
