...
When running in Kubernetes or a similar orchestrator, you can autoscale workers based on queue depth using the [Task Stats API](/v1/runtime/autoscaling-workers). Hatchet also supports [KEDA integration](/v1/runtime/autoscaling-workers#autoscaling-with-keda) for event-driven autoscaling.
...
| Concept                                               | What it does                                                            |
| ----------------------------------------------------- | ----------------------------------------------------------------------- |
| [Running with Docker](/v1/runtime/docker)               | Containerize workers for deployment.                                    |
| [Autoscaling Workers](/v1/runtime/autoscaling-workers)  | Scale workers dynamically based on queue depth.                         |
| [Worker Health Checks](/v1/observability/worker-healthchecks) | Expose `/health` and `/metrics` endpoints for monitoring.               |
...
If you haven't already, read about [tasks](/v1/runnables/tasks) to understand how work is defined and configured.
