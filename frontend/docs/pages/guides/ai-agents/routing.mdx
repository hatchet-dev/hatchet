...
| Routing rules are simple and known at definition time  | Use [Parent Conditions](/v1/durable-workflows/parent-conditions) in a DAG instead |

## How it maps to Hatchet

The router is a **[durable task](/v1/durable-workflows/durable-task-execution)**. It spawns a classifier [child task](/v1/durable-workflows/child-spawning) (or does classification inline), then spawns the matching specialist as a [child run](/v1/durable-workflows/child-spawning). Since each specialist is a separate durable task, they can have their own [timeouts](/v1/error-handling/timeouts), [retries](/v1/error-handling/retry-policies), [rate limits](/v1/flow-control/rate-limits), and [concurrency](/v1/flow-control/concurrency) settings.
...
- [Durable Task Execution](/v1/durable-workflows/durable-task-execution): understand how the router and specialists checkpoint
- [Child Spawning](/v1/durable-workflows/child-spawning): spawn specialist tasks from the router
- [Timeouts](/v1/error-handling/timeouts): set execution timeouts on the router and each specialist
- [Retry Policies](/v1/error-handling/retry-policies): configure retries for classification and specialist tasks
