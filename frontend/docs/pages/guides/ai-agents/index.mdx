...
| Reasoning loop      | [Child spawning](/concepts/durable-workflows/durable-task-execution/child-spawning): task re-spawns itself until done                                                           |
| Tool calls          | [Child tasks](/concepts/durable-workflows/directed-acyclic-graphs/child-spawning): sequential or [parallel](/concepts/durable-workflows/directed-acyclic-graphs/child-spawning) |
...
**Frees slots during waits.** When an agent waits for a human approval event, external event, or sleeps for a scheduled retry, the worker slot is [evicted](/concepts/durable-workflows/durable-task-execution/task-eviction) and freed. No resources are held while the agent is idle, even if the wait lasts hours or days.

**Handles streaming.** Pipe LLM tokens from inside the task to connected clients as they're generated. Hatchet manages the plumbing so you don't build your own pub/sub layer. See [Streaming](/concepts/streaming).

**Controls concurrency and rate limits.** Use [`CANCEL_IN_PROGRESS`](/concepts/concurrency) on a session key so new user messages cancel stale agent runs. Use [`GROUP_ROUND_ROBIN`](/concepts/concurrency) to distribute work fairly across users at scale. Add [rate limits](/concepts/rate-limits) to stay within external API quotas.
