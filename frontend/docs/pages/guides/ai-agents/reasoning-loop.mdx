...
| One-shot classification or extraction           | Skip: a single [task](/v1/runnables/tasks) is simpler                 |
...
Create a [durable task](/v1/durable-workflows/durable-task-execution) that invokes the reasoning loop from Step 1. Concurrency is set to `CANCEL_IN_PROGRESS` so new user messages cancel stale runs.
...
Emit LLM tokens from the task as they are generated. Clients subscribe to the stream and receive them in real-time. See [Streaming](/v1/advanced-tasks/streaming) for the full API.
...
<Callout type="warning">
  ...
  bounds, an agent can loop indefinitely. See [Timeouts](/v1/error-handling/timeouts) for
  configuration.
</Callout>
...
The evaluator-optimizer task loops: generate, evaluate, check score. Each generator and evaluator call is a [spawned child task](/v1/durable-workflows/child-spawning) that is checkpointed on completion.
...
- [Durable Workflows](/v1/durable-workflows/durable-task-execution): understand checkpointing and replay
- [Streaming](/v1/advanced-tasks/streaming): set up real-time LLM output streaming
- [Concurrency Control](/v1/flow-control/concurrency): configure CANCEL_IN_PROGRESS for chat agents
