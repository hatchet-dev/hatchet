...
Each specialist is a separate [durable task](/concepts/durable-workflows/durable-task-execution) with its own prompt, tools, timeout, and retry settings. The orchestrator's LLM decides which specialist to call next based on the accumulated context.
...
The orchestrator is a **[durable task](/concepts/durable-workflows/durable-task-execution)** running a reasoning loop via [child spawning](/concepts/durable-workflows/durable-task-execution/child-spawning). Each specialist is a standalone durable task spawned as a child run. The orchestrator's LLM returns structured tool calls, and each tool name maps to a specialist task.
...
The orchestrator runs a [reasoning loop](/guides/ai-agents/reasoning-loop): call LLM, parse tool choice, [spawn specialist](/concepts/durable-workflows/durable-task-execution/child-spawning), observe result, repeat. Context accumulates across iterations so later specialist calls have full history.
...
- [Durable Task Execution](/concepts/durable-workflows/durable-task-execution): understand checkpointing and replay for the orchestrator
- [Child Spawning](/concepts/durable-workflows/durable-task-execution/child-spawning): spawn specialist tasks from the orchestrator loop
- [Timeouts](/concepts/timeouts): set execution timeouts on the orchestrator and each specialist
- [Concurrency Control](/concepts/concurrency): limit how many orchestrator runs execute in parallel
