...
You can get access to the workflow's input data, such as the event data or other specified input data, by using the `context.workflowInput()` method on the `context`. For example, given the following event:
