...
Now, let's define some simple [pydantic](https://docs.pydantic.com) models for our Request body. Create a file `/backend/src/api/models.py` and define a `Message` and a `MessageRequest` model:
