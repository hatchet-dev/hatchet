...
# > Create a workflow
@hatchet.workflow(name="TaskConditionWorkflow")
class TaskConditionWorkflow:

    # > Add base task
    @hatchet.task(name="start")
    def start(self, context: Context) -> StepOutput:
        return StepOutput(random_number=random.randint(1, 100))

    # > Add multiply task
    @hatchet.task(name="multiply", parents=["start"], timeout="10s")
    def multiply(self, context: Context) -> StepOutput:
        time.sleep(10)
        return StepOutput(random_number=random.randint(1, 100) * 10)
# !!
```
...
```typescript
{
  "create_a_workflow": {
    ...
    "content": "@hatchet.workflow(name=\"TaskConditionWorkflow\")\nclass TaskConditionWorkflow:...",
  ...
  "add_base_task": {
    "title": "add_base_task",
    "content": "@hatchet.task(name=\"start\")\ndef start(self, context: Context) -> StepOutput:\n    return StepOutput(random_number=random.randint(1, 100))...",
    "githubUrl": "https://github.com/hatchet-dev/hatchet/tree/main/examples/python/workflows/complex-conditions.py",
    "language": "python",
    "codePath": "examples/python/workflows/complex-conditions.py"
  }
...
<Snippet src={snippets.python.workflows.complex_conditions.create_a_workflow} />

Then add your base task that generates a random number:

<Snippet src={snippets.python.workflows.complex_conditions.add_base_task} />
```
