...
<div className="w-full pb-4 mx-auto border-b shadow-md flex flex-row justify-between items-center mt-10">
  ...
  <p className="font-light text-foreground">Published on May 31st, 2024</p>
</div>

***Disclosure:** I'm a co-founder of Hatchet, a multi-language task queue with Python support. We're open-source (with a cloud version) and we aim to be a drop-in replacement for Celery that supports a modern Python stack.*

Most people who build webapps in Python have encountered Celery. It's described as a “distributed task queue” — which basically means that it helps users divide background tasks between a set of processes, called workers.
...
While Celery has gotten *a lot* right, there are many potential pitfalls when using Celery. These come from personal experience — Alexander's perspective being on the infrastructure side, Gabe's being on the application side — along with migrating a number of Celery users to Hatchet. We've grouped these issues into 3 categories:
...
Celery doesn't support running async functions as task out of the box, and also doesn't allow created tasks to be awaited. Instead, you'll have to rely on workarounds like polling for a task result or [converting async methods to synchronous ones](https://github.com/celery/celery/issues/6552#issuecomment-1870832344).
...
There have been some mentions of this being available in Celery 6.0 or Q3/Q4 2024, but there isn't much information outside of this. There is an [open issue](https://github.com/celery/celery/issues/7874) from an external contributor with a barebones implementation of an AsyncIO pool which seems promising. Another promising solution looks to be  ‣,
...
This isn't possible in Celery — only rate limits per worker are supported. The docs state the following:
...
*A quick aside — this behavior will vary depending on which broker is used. RabbitMQ will round-robin across many FIFO queues. [This doesn't seem to occur in Redis](https://github.com/celery/celery/issues/2192#issuecomment-1250358406).*
...
Note that clock synchronization is only important in Celery when using ETA/countdown features, which you probably shouldn't be using anyway (see below).
...
1. The decision to set this default, along with the decision to not surface an error when a worker restarts and reconnects, makes programming *more* difficult. Debugging behaviors involving the absence of a task is much more difficult than debugging failures involving multiple task executions - the absence of a task execution (or a task in a perpetual “Started” state according to [celery.events](http://celery.events)) lives outside the purview of the application, and won't be captured by typical logs or failure events instrumented by the application developer.
2. It's far more common for a task queue to implement at-least-once execution with idempotency than at-most-once execution. If we wanted at-most-once execution, the simplest approach would be a pub/sub system without message persistence.
...
As a result, it's usually recommended to set **`worker_prefetch_multiplier`** to 1 in Celery.
...
- *Which tasks failed in a certain time range?*
- *When did I see a spike in task latency?*
- *Do any workers have a high task failure rate?*
...
And this doesn't even include basic alerting, which would involve setting up triggers in Honeycomb or configuring alert-manager rules with Grafana.
...
If you've read this far, perhaps you'd be interested in how we're solving some of these problems with Hatchet. The reason we can avoid certain classes of problems is that we have a central component (our “engine”) along with a Postgres database for result persistence, which makes it much easier to implement global rate limits (managed by our central component) and also lets us persist ETA tasks instead of having to pull them off the queue immediately.

Observability with dashboards and alerting is also a first-class citizen of the Hatchet project. We currently support Slack and email-based alerts on failure, and we have a feature-rich UI which lets you debug and filter your tasks:

{/* <screenshot> */}

If you'd like to give it a try, you can get up and running in 5 minutes with Hatchet Cloud.
