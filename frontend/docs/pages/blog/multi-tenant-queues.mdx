...
<div className="w-full pb-4 mx-auto border-b shadow-md flex flex-row justify-between items-center mt-10">
  ...
  <p className="font-light text-foreground">Published on April 15, 2024</p>
</div>
...
Let's start with some code! We're implementing a basic Postgres-backed task queue, where workers poll for events off the queue at some interval. You can find all the code used in these examples — along with some nice helper `seed` and `worker` commands — in this repo: [github.com/abelanger5/postgres-partitioned-queue](https://github.com/abelanger5/postgres-partitioned-queue). Note that I chose `sqlc` to write these examples, so you might see some `sqlc.arg` and `sqlc.narg` in the example queries.
