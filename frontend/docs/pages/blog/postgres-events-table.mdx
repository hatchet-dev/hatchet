...
- [Exposing state transitions to users](#state-transitions)
...
Note the single index on `(resource_id, tenant_id)` — we don't want too many indexes so we can reduce the write overhead on this query. To query this, simply query by the resource ID and tenant ID, ordered by the primary key:
...
Note that Postgres is not a good fit if you need to construct arbitrary aggregate queries which are either ad-hoc or user-defined. If you need true OLAP at scale, check out [Clickhouse](https://clickhouse.com/).
...
Additionally, you should set a cleanup policy on your hypertables. This is available in Timescale via `add_retention_policy` — for example, to limit retention to 24 hours:
