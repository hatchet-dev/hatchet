...
With Hatchet, you can perform all of these requests in a single task concurrently, as opposed to needing to e.g. enqueue a single task per request. This is easier to program, easier to reason about, more performant on your (the client) side, and also puts less pressure on the backing queue, since it needs to handle an order of magnitude fewer requests in this case.
...
If you've made it this far, try us out! You can get up and running in five minutes on [Hatchet Cloud](https://cloud.onhatchet.run/). And if you'd like to learn more, can find us:
