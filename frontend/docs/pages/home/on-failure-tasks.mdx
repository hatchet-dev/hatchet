...
export const getStaticProps = ({}) => getSnippets([OnFailureGo]);
...
<UniversalTabs items={["Python", "Typescript", "Go"]}>
  <Tabs.Tab title="Python">
    <Snippet src={snips.python.on_failure.worker.onfailure_step} />
    <Callout variant="warning">
      Note: Only one on-failure task can be defined per workflow.
    </Callout>
  </Tabs.Tab>
  <Tabs.Tab title="Typescript">
    <Snippet src={snips.typescript.on_failure.workflow.on_failure_task} />
  </Tabs.Tab>
  <Tabs.Tab title="Go">
