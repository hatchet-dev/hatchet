...
Durable tasks run on the same worker process as regular tasks, but they use a separate durable slot pool. The worker advertises both `slots` and `durable_slots`, and the engine routes durable tasks to the durable slot group so long-running work does not consume standard slots.
...
<Callout type="info" emoji="ℹ️">
  If you're upgrading from older SDKs that registered a separate `-durable`
  worker, Hatchet still recognizes those legacy workers as durable slot
  capacity. New SDKs register a single worker with durable slots instead.
</Callout>
