...
export const getStaticProps = ({}) => getSnippets([SimpleTs, SimplePy, SimpleGo, FanoutPy, ParentTaskGo]);
...
<Callout type="info">
 ...
 You can use the `run` method directly to bulk run tasks by passing an array of inputs. This will return a list of results.
  ...
  Available bulk methods on the `Context` object are:
    - `bulkRunChildren`
    - `bulkRunChildrenNoWait`
