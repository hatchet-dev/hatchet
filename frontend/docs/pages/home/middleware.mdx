...
<UniversalTabs items={["Python", "Typescript", "Go", "Ruby"]}>
  ...
  <Tabs.Tab title="Typescript">
    Create a client and attach middleware with `pre` and `post` hooks.

    - **`pre(input, ctx)`** runs before the task. Its return value **replaces** the task input.
    - **`post(output, ctx, input)`** runs after the task. Its return value **replaces** the task output.

    <Snippet src={snippets.typescript.middleware.client.init_a_client_with_middleware} />

    <Callout type="warning" emoji="⚠️">
      **Spread the original value if you want to keep it.** The return value of each hook **replaces** the input (or output) entirely — it does not shallow-merge. If you omit `...input` in a `pre` hook, the original fields are lost. The same applies to `...output` in a `post` hook.

      ```typescript
      // ✅ Keeps original fields and adds `requestId`
      pre: (input) => ({ ...input, requestId: crypto.randomUUID() })

      // ❌ Replaces input entirely — task only receives { requestId }
      pre: (input) => ({ requestId: crypto.randomUUID() })
      ```
    </Callout>

    ### Chaining Middleware

    You can chain multiple `.withMiddleware()` calls to run hooks in sequence. Each `pre` hook receives the return value of the previous `pre` hook (or the original input for the first hook), and each `post` hook receives the return value of the previous `post` hook.
...
<UniversalTabs items={["Python", "Typescript", "Go", "Ruby"]}>
  ...
  <Tabs.Tab title="Typescript">
    ...
    Each `pre` hook receives the current input and the task `Context`. Its return value **replaces** the input for the next hook (or the task itself). Returning `undefined` (or `void`) skips replacement and passes the input through unchanged.

    ### The task function executes

    The task receives the final input after all `pre` hooks have run.

    ### Post hooks run in registration order

    Each `post` hook receives the current output, the task `Context`, and the final input. Its return value **replaces** the output for the next hook (or the final result). Returning `undefined` skips replacement.
    </Steps>

    Both `pre` and `post` hooks can be **async** — return a `Promise` and it will be awaited before proceeding.
    ...
    The second parameter of both `pre` and `post` hooks is the task `Context` object. This gives middleware access to:
    ...
    | **Middleware pre hook** | `.withMiddleware({ pre })` | No — injected automatically by the worker | Yes |
    ...
    Middleware `pre` hooks, on the other hand, inject fields that are **computed at runtime** (e.g. request IDs, decrypted secrets, fetched config) and are **not** required from callers.
...
<UniversalTabs items={["Python", "Typescript", "Go", "Ruby"]}>
  ...
  <Tabs.Tab title="Typescript">
    ...
    The task's `input` type is the intersection of `TaskInput`, `GlobalInputType`, and the return type of the `pre` middleware hook. The task's return type must satisfy `TaskOutput` and `GlobalOutputType`, while the caller receives the intersection of those with the `post` middleware return type.
...
<Callout type="info">
  The `pre` hook decrypts incoming data so your task function works with
  plaintext. The `post` hook encrypts the output before it is stored. The
...
<Callout type="warning" emoji="⚠️">
  The caller is responsible for uploading oversized inputs to S3 before triggering the task. The `pre` hook only handles the download side. You can use the same `uploadToS3` helper on the caller side to upload the input and pass `{ __s3Url: url }` as the task input.
</Callout>
...
Hatchet middleware runs **inside the worker process** around each task invocation — not on an HTTP request path. A `pre` hook transforms input before the task runs, and a `post` hook transforms output after. Unlike Express middleware, there is no `next()` function; hooks return their result directly and the runner chains them automatically.
...
If a `pre` or `post` hook throws (or returns a rejected `Promise`), the task run fails with that error. There is no automatic retry of middleware itself, but the task's configured retry policy will still apply, re-running the task (and its middleware) from scratch.

### Can I use async/await in middleware hooks?

Yes. Both `pre` and `post` hooks can be synchronous or asynchronous. If a hook returns a `Promise`, the worker will `await` it before proceeding to the next hook or the task function.

### How do I share state between `pre` and `post` hooks?

The `post` hook receives the task input (after `pre` hooks have run) as its third argument. Add fields in `pre` (e.g. `startedAt`, `traceId`) and read them from `input` in `post`. There is no separate shared context object — the input itself is the carrier.
