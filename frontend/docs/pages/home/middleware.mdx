...
<UniversalTabs items={["Python", "Typescript", "Go", "Ruby"]}>
  ...
  <Tabs.Tab title="Typescript">
    ...
    You can chain multiple `.withMiddleware()` calls to run hooks in sequence. Each hook's return value is merged into the input for subsequent hooks and the task itself.
