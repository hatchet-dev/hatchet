...
<Callout type="info">
  ...
  `hatchet.runs.bulk_cancel` with `hatchet.runs.bulk_replay`.
    ...
    <Snippet src={snips.python.bulk_operations.cancel["List runs"]} />

    And finally, we can cancel the runs in bulk.

    <Snippet src={snips.python.bulk_operations.cancel['Cancel by run ids']} />
...
{/* <UniversalTabs items={["Python", "Typescript", "Go"]}> TODO V1 DOCS - Add TS and Go */}
  <Tabs.Tab title="Python">
    ...
    <Snippet src={snips.python.bulk_operations.cancel["Cancel by filters"]} />
