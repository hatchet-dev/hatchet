...
<Callout type="warning" emoji="⚠️">
  We recommend only using lifespans for sharing **_immutable_** state. The
  intention is not to e.g. store a dictionary that has the count of tasks that a
  worker has run and use the lifespan to increment that counter on each task
  run. This is prone to odd behavior due to concurrency in Hatchet, so you'd
  need to be careful about using locks / mutexes for shared state mutation like
