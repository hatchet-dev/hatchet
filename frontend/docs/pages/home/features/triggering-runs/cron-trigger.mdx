...
Hatchet supports cron triggers to run on a schedule defined in a few different ways:
 - [Workflow Definitions](./cron-trigger.mdx#defining-a-cron-in-your-workflow-definition): Define a cron expression in your workflow definition to trigger the workflow on a predefined schedule.
 - [Dynamic Programatically](./cron-trigger.mdx#programatically-creating-cron-triggers): Use the Hatchet SDKs to dynamically set the cron schedule of a workflow (i.e. customer defined schedule).
 - [Hatchet Dashboard](./cron-trigger.mdx#managing-cron-jobs-in-the-hatchet-dashboard): Manually create cron triggers from the Hatchet Dashboard.
