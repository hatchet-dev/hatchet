...
export const OnFailureSrc = { path: 'src/examples/on-failure.ts' };
export const BulkTriggerSrc = { path: 'src/examples/bulk-trigger.ts' };

export const getStaticProps = ({ params }) => getSnippets(
  [
    OnFailureSrc,
    BulkTriggerSrc
  ]
...
<GithubSnippet src={OnFailureSrc} target="OnFailure Step" />
