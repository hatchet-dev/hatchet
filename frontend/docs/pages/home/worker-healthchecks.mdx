...
1. `/health` - Returns **200** when the worker listener is healthy, otherwise **503**. The response is intentionally minimal because the endpoint is public: `{"status":"HEALTHY"}` or `{"status":"UNHEALTHY"}`.
