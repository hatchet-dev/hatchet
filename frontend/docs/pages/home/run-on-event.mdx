...
<UniversalTabs items={["Python", "Typescript", "Go"]}>
  ...
  <Tabs.Tab title="Typescript">
    <Snippet
      src={snips.typescript.on_event.filter}
      block="create_a_filter"
    />
  </Tabs.Tab>
  <Tabs.Tab title="Go">
...
<Callout type="info">
 In Hatchet, filters are "positive", meaning that we look for _matches_ to the filter to determine which tasks to trigger,
</Callout>
