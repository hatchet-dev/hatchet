...
<Callout type="info">
  Additional metadata can be added to `Runs`, `Scheduled Runs`, `Cron Runs`, and `Events`.
  The data is propagated from parents to children or from events to runs.
</Callout>
