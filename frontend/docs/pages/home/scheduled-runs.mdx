...
<Callout type="warning">
  You can only reschedule scheduled runs created via the API (not runs created via
  a code-defined schedule), and Hatchet may reject rescheduling if the run has
  already triggered.
</Callout>
