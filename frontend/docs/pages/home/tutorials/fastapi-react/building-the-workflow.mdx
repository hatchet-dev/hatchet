...
Next, let's add a step to read in the url from the workflow input, load the contents of that page with `requests` and parse the html content to text with `Beautiful Soup`.
