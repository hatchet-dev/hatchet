...
- [`GROUP_ROUND_ROBIN`](./group-round-robin.mdx): Distribute workflow instances across available slots in a round-robin fashion based on the `key` function.
- [`CANCEL_IN_PROGRESS`](./cancel-in-progress.mdx): Cancel the currently running workflow instances for the same concurrency key to free up slots for the new instance.
