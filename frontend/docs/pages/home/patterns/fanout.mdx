...
<PatternComparison
  rows={[
    { label: "How it works", workflow: "Parent task spawns children via RunChild and awaits results", durable: "Same — RunChild is checkpointed in the durable event log" },
    ...
    { label: "Crash recovery", workflow: "Parent retries from scratch if it fails mid-fanout", durable: "Parent replays from checkpoint — already-spawned children are not re-spawned" },
    { label: "Best for", workflow: "Small-to-medium fan-outs where parent completes quickly", durable: "Large fan-outs (hundreds/thousands of children) or when children take a long time" },
  ]}
