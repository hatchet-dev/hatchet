...
const batch = hatchet.batchTask({
  name: "simple",
  batchMaxSize: 2,
  batchMaxInterval: "1s", // defaults to 1000ms if omitted
  batchGroupKey: "input.batchId", // defaults to `'default'` if omitted
  batchGroupMaxRuns: 100, // defaults to 100 if omitted
