...
<Callout type="warning">
  NOTE: You must first [register the task on a worker](./workers.mdx) before you can run it.
  Calling `your_task.run` will enqueue a task to be executed by a worker but it will wait indefinitely for the task to be executed.
</Callout>
