...
<Callout type="warning">
  ...
  strategy](./assignment_sticky-assignment.mdx). In this case, it is recommended
  to set desired labels on the first step of the workflow to ensure that the
  workflow is assigned to a worker that meets the desired criteria and remains
  on that worker for the duration of the workflow.
</Callout>
