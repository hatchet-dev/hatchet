...
| --------------------------------------------------------- | ------------------------------------------------------------------------------------------ |
| **[Durable Execution](/guide/durable-workflows-overview)** | Checkpoints tool calls, LLM responses, and loop iterations so agent state survives crashes |
| **[Streaming](/features/streaming)**                      | Stream LLM tokens from workers to frontends in real-time                                   |
| **[Durable Events](/features/durable-events)**            | Pause for human-in-the-loop feedback without holding worker slots                          |
| **[Durable Sleep](/features/durable-sleep)**              | Schedule agent retries or delayed actions with exact timing                                |
| **[Concurrency](/features/concurrency)**                  | Cancel stale runs, limit parallel agents per user                                          |
| **[Rate Limits](/features/rate-limits)**                  | Stay within LLM provider API rate limits across all workers                                |
| **[Child Spawning](/features/child-spawning)**            | Agents dynamically spawn sub-agents or tool-call tasks                                     |
| **[Cancellation](/features/cancellation)**                | Stop long-running agents on user request                                                   |

<Callout type="info">
  The agent loop pattern is a [Cycle](/guide/patterns/cycles). Each iteration
  spawns a new child run via `RunChild`. Combined with [Durable
  Execution](/guide/durable-workflows-overview), completed iterations survive
  crashes and slots are freed between iterations.
</Callout>
...
<Callout type="warning">
  ...
  bounds, an agent can loop indefinitely. See [Timeouts](/features/timeouts) for
  configuration.
</Callout>
...
<Cards>
  ...
  <Cards.Card title="Long Waits" href="/guide/patterns/long-waits">
  ...
  <Cards.Card title="Branching" href="/guide/patterns/branching">
...
- [Durable Workflows](/guide/durable-workflows-overview): understand checkpointing and replay
- [Streaming](/features/streaming): set up real-time LLM output streaming
- [Concurrency Control](/features/concurrency): configure CANCEL_IN_PROGRESS for chat agents
- [Child Spawning](/features/child-spawning): spawn tool-call tasks from agent loops
