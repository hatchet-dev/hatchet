...
You write simple functions, called [tasks](/guide/your-first-task), in Python, Typescript, and Go and run them on [workers](/guide/workers) in your own infrastructure. You can compose these tasks into [parent/child relationships](/features/child-spawning) or predefined as [Directed Acyclic Graphs (DAGs)](/features/dags) to build more complex pipelines, which we call [workflows](/features/orchestration). All tasks and workflows are **defined as code**, making them easy to version, test, and deploy.
...
- **AI agents**: a core number of Hatchet's features, like [webhooks](/features/webhooks), [child spawning](/features/child-spawning), and [dynamic workflows](/features/child-spawning) are designed to support AI agents.
- **Event-driven systems**: Hatchet's [eventing features](/features/run-on-event) allow you to build event-driven architectures without requiring additional infrastructure.

## Why Hatchet?

⚡️ **Low-Latency For Real-Time Workloads** - Sub-25ms task dispatch for hot workers with thousands of concurrent tasks. Smart assignment rules handle [rate-limits](/features/rate-limits), [fairness](/features/concurrency), and [priorities](/features/priority) without complex configuration.

🪨 **Durability for Long Running Jobs** - Every task invocation is durably logged to PostgreSQL. With [durable execution](/features/durable-execution), when jobs fail your workflow will resume exactly where you left off — no lost work, no duplicate LLM calls, no engineer headaches.
...
- [Hatchet Cloud Quickstart](/guide/hatchet-cloud-quickstart)
- [Hatchet Self-Hosted Quickstarts](/guide/self-hosting)
...
**[Architecture & Guarantees](/guide/what-is-hatchet/architecture-and-guarantees)** - Learn how Hatchet is built, its guarantees, and when to use it.

Or get started with the **[Hatchet Cloud Quickstart](/guide/hatchet-cloud-quickstart)** or **[self-hosting](/self-hosting)**.
