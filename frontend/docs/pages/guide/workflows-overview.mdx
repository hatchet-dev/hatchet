...
Each task has configurable [retry policies](/features/retry-policies) and [timeouts](/features/timeouts). If a task fails, Hatchet retries it without re-running already-completed tasks.
...
| **Trigger**    | How a workflow starts: [direct run](/guide/run-with-results), [event](/guide/run-on-event), [cron](/guide/cron-runs), [schedule](/guide/scheduled-runs), or [webhooks](/guide/webhooks) |
...
<Cards>
  <Cards.Card title="Durable Workflows" href="/guide/durable-workflows-overview">
    Learn how durable workflows checkpoint, pause, and resume across failures.
  </Cards.Card>
  <Cards.Card title="DAGs in Detail" href="/features/dags">
    Define multi-task workflows with dependencies, parent outputs, and parallel
    execution.
  </Cards.Card>
  <Cards.Card title="Child Spawning" href="/features/child-spawning">
    Dynamically create child tasks at runtime for fan-out, loops, and agent
    patterns.
  </Cards.Card>
  <Cards.Card title="Patterns" href="/guide/patterns/fanout">
