...
As we continue to build with Hatchet, we'll add additional configuration options to compose tasks into [DAG workflows](/features/dags) or [procedural child spawning](/features/child-spawning).
...
The returned object is an instance of the `Task` class, which is the primary interface for interacting with the task (i.e. [running](/features/run-with-results), [enqueuing](/features/run-no-wait), [scheduling](/features/scheduled-runs), etc).
...
- [Running a task with results](/features/run-with-results)
- [Enqueuing a task](/features/run-no-wait)
- [Scheduling a task](/features/scheduled-runs)
- [Scheduling a task with a cron schedule](/features/cron-runs)
- [Event-driven task execution](/features/run-on-event)
