...
Dev variants exist for the other server images too: `hatchet-api-dev`, `hatchet-engine-dev` and `hatchet-admin-dev` (`ghcr.io/hatchet-dev/hatchet/hatchet-<component>-dev`). They are for local development and testing only - the regular images never compile the embedded token in, so it is inert against any normal deployment.
