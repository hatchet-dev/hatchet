...
| `options?` | [`SleepForOptions`](#sleepforoptions) | Optional configuration for the sleep. |
...
<a id="sleepforoptions"></a>

#### SleepForOptions

Options for the `sleepFor()` method.

| Property | Type | Description |
| -------- | ---- | ----------- |
| `readableDataKey?` | `string` | Optional key used for condition result indexing. |
| `label?` | `string` | Optional human-readable label shown in the dashboard run details. Use this to provide context about what the task is waiting for (e.g., "Waiting for payment confirmation"). |
...
| `scope?` | `string` | An optional scope to narrow down event matching. Required when using `lookbackWindow`. |
| `lookbackWindow?` | `Duration` | An optional lookback window to check for events that arrived before the wait was established. Requires `scope` to be set. |
| `label?` | `string` | Optional human-readable label shown in the dashboard run details. Use this to provide context about what the task is waiting for. |
