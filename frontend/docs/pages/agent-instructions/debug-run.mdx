...
- `.tasks[]` -- each task in the run, with its own `status`, `displayName`, and `output` or `error`
...
- `eventType` -- what happened (e.g. `TASK_QUEUED`, `TASK_STARTED`, `TASK_COMPLETED`, `TASK_FAILED`, `TASK_CANCELLED`)
...
### Task stuck in QUEUED (no TASK_STARTED event)
...
The events from Step 2 will show a `TASK_FAILED` event with an error message.

### Task CANCELLED

Check the events for a `TASK_CANCELLED` event. The message field indicates the cancellation source:
