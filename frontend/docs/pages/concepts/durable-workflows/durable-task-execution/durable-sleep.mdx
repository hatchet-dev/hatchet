...
Durable sleep is a feature of **durable tasks** which pauses execution for a specified amount of time. A long wait pauses a task for hours, days, or weeks and frees the worker slot until the sleep expires — no resources are consumed during the wait.

<LongWaitDiagram />

<Callout type="info">
  Sleeping puts the task into an [evictable
...
<Callout type="info">
  DAG workflows have an analogous feature: [Sleep
  Conditions](/concepts/durable-workflows/directed-acyclic-graphs/sleep-conditions).
  In a DAG, sleep conditions are declared upfront on the task definition. Use a
  durable task when the decision to sleep is made dynamically at runtime.
</Callout>
