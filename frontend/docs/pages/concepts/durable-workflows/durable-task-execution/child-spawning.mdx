...
<Callout type="info">
  Waiting for child results puts the parent task into an [evictable state](/concepts/durable-workflows/durable-task-execution/task-eviction) — the worker slot is freed and the parent is re-queued when results are available.
</Callout>
...
<Callout type="info">
  For the general child spawning API (task definitions, options, deduplication), see [Procedural Child Task Spawning](/concepts/durable-workflows/directed-acyclic-graphs/child-spawning).
</Callout>
