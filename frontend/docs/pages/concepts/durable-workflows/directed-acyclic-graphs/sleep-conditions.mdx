...
Sleep conditions are the DAG equivalent of long waits. While durable tasks pause dynamically at runtime with `SleepFor`, DAG sleep conditions declare the delay upfront as part of the task definition. Both free the worker slot during the wait.
