...
<Snippet src={snippets.go.streaming.shared.task.streaming} />
  ...
  <Tabs.Tab title="Ruby">
    <Snippet src={snippets.ruby.streaming.worker.streaming} />
  </Tabs.Tab>
</UniversalTabs>
...
You can easily consume stream events by using the stream method on the workflow run ref that the various [fire-and-forget](/concepts/run-no-wait) methods return.
...
<Snippet src={snippets.go.streaming.consumer.main.consume} />
  ...
  <Tabs.Tab title="Ruby">
    <Snippet src={snippets.ruby.streaming.async_stream.consume} />
  </Tabs.Tab>
</UniversalTabs>
