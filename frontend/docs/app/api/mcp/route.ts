...
function trackMcpEvent(
  ...
  const sessionId = headers.get("mcp-session-id") || "anonymous";
  ph.capture({
...
function routeRequest(
  ...
  trackMcpEvent(headers, method, trackProps);
...
export async function POST(req: Request) {
  ...
  if (Array.isArray(body)) {
    ...
    for (const item of body) {
      const result = routeRequest(item as JsonRpcRequest, req.headers);
