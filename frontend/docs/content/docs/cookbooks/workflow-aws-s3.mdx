---
title: "S3 Processing Pipeline"
seoTitle: "Process S3 Objects at Scale: a Parallel Pipeline | Hatchet"
...
import { S3Pipeline } from "@/components/flow-animations/S3Pipeline";
...
These two concurrency entries compose to ensure a **single poller per bucket** paired with a **workflow-level limitation** on total in-flight pollers.
...
The diagram above shows this steady state: work interleaves across buckets, the worker pool holds at its slot cap, and the backlog drains in round-robin order. Even when `bucket-1` has the most pending objects, it doesn't monopolize the workers.
