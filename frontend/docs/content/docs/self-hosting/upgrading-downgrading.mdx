...
import { Callout, Steps } from "@/components/nextra-compat";
...
<Callout type="warning">
  Upgrading to `v0.87.6` or later requires the `btree_gist` Postgres extension.
  Some managed providers, such as Azure Database for PostgreSQL, require you to
  allow-list it before upgrading. See [Required Postgres
  extensions](/self-hosting/kubernetes-external-database#required-postgres-extensions).
</Callout>
