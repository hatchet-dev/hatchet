---
title: "Quickstart"
seoTitle: "Kubernetes Quickstart"
---

import { Tabs, Steps, Callout } from "@/components/nextra-compat";
...
Encryption keys and secrets are generated automatically on first install by the quickstart job, which is enabled by default (`api.quickstartJob.enabled=true`). See [Credentials & Key Generation](/self-hosting/kubernetes-helm-configuration) for details.
...
Now that you have an API token, see the guide [here](/v1/quickstart) for how to run your first task.
