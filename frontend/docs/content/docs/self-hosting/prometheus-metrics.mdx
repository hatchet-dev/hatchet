...
To enable Prometheus metrics for your Hatchet instance, you can set the following environment variables. The corresponding configuration YAML values are mentioned in parentheses. If you are deploying [Hatchet in HA mode](/self-hosting/high-availability), these should be set on the `grpc`, `controllers`, and `scheduler` deployments.

<Callout type="warning">
  The [global metrics](#global-metrics) are per-process counters — every engine
  ...
  queries that reference created tasks (e.g. _Queue Processing Efficiency:
  Assigned vs Created_) cannot be computed. Make sure your scrape configuration
...
<Callout type="info">
  This step requires communication with a service that scrapes Hatchet
  Prometheus metrics.
</Callout>
...
##### Running more than one engine pod

The [tenant metrics](#tenant-metrics) are per-process: each engine pod only
counts the events it handled. The tenant endpoint therefore queries your
Prometheus for `sum without (pod) (...)` over every series carrying the tenant's
`tenant_id`, so pods that differ only by a `pod` label are summed into one series
per metric. Summing is correct for every tenant metric: counters and histograms
are counted on exactly one pod, and the gauges are reported by the single pod
that runs the tenant's scheduler.

With a default scrape config Prometheus tells engine pods apart by `instance`,
not `pod`, so the endpoint returns one series per instance and consumers must
aggregate themselves, for example `sum without (instance) (...)`. To serve one
series per tenant instead, give every engine pod the same `instance` value and
carry the pod identity in `pod`:

```yaml
scrape_configs:
  - job_name: hatchet-engine
...
<Callout type="warning">
  Do not collapse `instance` without adding `pod`. The pods' series then collide
  in Prometheus and the endpoint alternates between the totals of whichever pod
  was scraped last.
</Callout>

Summed counters still drop when a pod restarts, so `rate()` over the endpoint's
series shows a one-window spike at each rollout.
...
| `hatchet_pubsub_nats_scheduler_partition_drops_total` | Counter   | Messages dropped client-side by the NATS scheduler-partition subscription         |
...
`hatchet_pubsub_nats_scheduler_partition_drops_total` exists only when NATS
backend is enabled and covers the scheduler-partition subscription only. It is
read from the NATS client's `Subscription.Dropped()` at scrape time. Drops
happen when a subscription exceeds the nats.go default pending limits (500,000
messages / 64 MiB per subscription). For tenant-stream subscriptions the
...
The worker slot and queue size gauges are designed to drive autoscalers — see [Autoscaling Workers](/v1/autoscaling-workers#scaling-based-on-worker-slot-utilization) for how to use them with KEDA.
