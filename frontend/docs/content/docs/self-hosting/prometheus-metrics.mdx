...
`hatchet_pubsub_nats_client_drops_total` exists only with the NATS backend and
is labelled by `topic_kind` alone. It is read from the NATS client's
per-subscription drop counter (`Subscription.Dropped()`) and keeps
accumulating across subscription churn. Drops happen when a subscription
exceeds the nats.go default pending limits (500,000 messages / 64 MiB per
subscription).
