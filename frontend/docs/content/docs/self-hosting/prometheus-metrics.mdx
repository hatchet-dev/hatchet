...
| `hatchet_pubsub_nats_client_drops_total`  | Counter   | Messages dropped client-side by the NATS scheduler-partition subscription         |
...
`hatchet_pubsub_nats_client_drops_total` exists only when NATS backend is
enabled and covers the scheduler-partition subscription only. It is read from
the NATS client's `Subscription.Dropped()` at scrape time. Drops happen when a
subscription exceeds the nats.go default pending limits (500,000 messages /
64 MiB per subscription). For tenant-stream subscriptions the sampled
`nats pubsub async error` warning log is the only drop signal.
