...
| `hatchet_scheduling_latency_p50_seconds`  | Gauge     | True p50 of queued-to-assigned time over a 30s sliding window (NaN when idle)     |
| `hatchet_scheduling_latency_p99_seconds`  | Gauge     | True p99 of queued-to-assigned time over a 30s sliding window (NaN when idle)     |
| `hatchet_scheduling_latency_samples_dropped_total` | Counter | Latency samples dropped when a second exceeds the quantile window capacity |
...
| `hatchet_pubsub_nats_client_drops_total`  | Counter   | Messages dropped client-side by NATS pub/sub subscriptions (slow consumer)        |
| `hatchet_pubsub_nats_pending_msgs`        | Gauge     | Messages buffered client-side, max across NATS subscriptions of the topic kind    |
| `hatchet_pubsub_nats_pending_bytes`       | Gauge     | Bytes buffered client-side, max across NATS subscriptions of the topic kind       |
| `hatchet_pubsub_nats_pending_limit_msgs`  | Gauge     | Client-side pending message limit of NATS pub/sub subscriptions                   |
| `hatchet_pubsub_nats_pending_limit_bytes` | Gauge     | Client-side pending bytes limit of NATS pub/sub subscriptions                     |
...
The `hatchet_pubsub_nats_*` metrics exist only with the NATS backend and are
labelled by `topic_kind` alone. The drops counter is read from the NATS
client's per-subscription drop counter and keeps accumulating across
subscription churn; the pending gauges report the hottest live subscription of
each topic kind, so
`hatchet_pubsub_nats_pending_msgs / hatchet_pubsub_nats_pending_limit_msgs`
(and the bytes equivalent) is the client-side buffer occupancy — values
approaching 1 mean the subscriber is about to drop messages.
