...
each topic kind. Hatchet uses the nats.go default pending limits (500,000
messages / 64 MiB per subscription), so pending values approaching those
limits mean the subscriber is about to drop messages.
