...
| `SERVER_OTEL_COLLECTOR_HEADERS` (`otel.collectorHeadersString`) | Comma-separated `key=value` pairs sent as custom OTLP headers (e.g. `"X-API-Key=foo,X-Tenant=bar"`). Parsed at startup. If `Authorization` is set here it overrides `SERVER_OTEL_COLLECTOR_AUTH`. | empty    |
