...
<Snippet src={snippets.typescript.on_event.event.bulk_push_events} />
...
<Callout type="info">
  These are defaults. Some internal subscribers pin their own values: the shared
  tenant reader fixes its flush interval and concurrency, so only
  `SERVER_DEFAULT_BUFFER_SIZE` affects it.
</Callout>
