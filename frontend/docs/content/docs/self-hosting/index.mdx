...
The fastest way to get a Hatchet instance running locally is with the [Hatchet CLI](/reference/cli) (which wraps Hatchet Lite):

```sh
hatchet server start
```

For local development, see [Running Hatchet Locally](/v1/running-locally) for lighter options, including embedded mode with no Docker or token.
