...
| `DATABASE_FAIR_TENANT_POOL_MAX_PERCENT` | Percent of `DATABASE_MAX_CONNS` one tenant may hold. Shared engine work (partition maintenance, heartbeats, cross-tenant flushes) is capped at the same percent in its own bucket, reported as `tenant_id="shared"` on the gate metrics. `100` installs no cap. Values from `1` to `99` install the cap. | `100` |
| `DATABASE_TENANT_POOL_MAX_WAIT` | How long an acquire waits for a free tenant slot before failing. Applies when the percent is from `1` to `99`. | `5s` |
