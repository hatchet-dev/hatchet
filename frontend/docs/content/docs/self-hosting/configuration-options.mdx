...
| `SERVER_OTEL_COLLECTOR_HEADERS`          | Comma-separated `key=value` pairs of custom OTLP headers (e.g. `X-API-Key=foo,X-Tenant=bar`). An explicit `Authorization` here overrides `SERVER_OTEL_COLLECTOR_AUTH`. |               |
