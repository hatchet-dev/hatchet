...
| `DATABASE_ENABLE_JIT`           | Enable PostgreSQL JIT compilation for queries known to trigger JIT-related backend crashes. When `false`, those queries run with `SET LOCAL jit = off` in their transaction. When `true`, the server's `jit` setting is left untouched.                                      | `false`                                |
