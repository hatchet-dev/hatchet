...
| `SERVER_GRPC_OPERATORS_ENABLED`                        | Register the gRPC operator API (`OperatorService`) for operators running outside the engine process                                                   | `false`                 |
| `SERVER_GRPC_OPERATOR_MAX_LISTEN_STREAMS_PER_OPERATOR` | Max Listen streams one gRPC operator may hold open on an engine replica; each stream is a live worker. `0` disables the cap                           | `100`                   |
| `SERVER_GRPC_OPERATOR_MAX_ACTIONS_PER_OPERATOR`        | Max action links held across all workers of one gRPC operator; a delta over the cap is refused. `0` disables the cap                                  | `1000000`               |
| `SERVER_DAG_OPERATOR_DEFAULT_SLOTS`                    | Durable slots the DAG operator's worker advertises when the operator's config sets no `slots` (how many DAG runs one dispatcher orchestrates at once) | `10000`                 |
| `SERVER_SERVERLESS_OPERATOR_ENABLED`                   | Run the serverless operator inside the engine (needs `SERVER_OPERATOR_INFRA_BLOCKED_CIDRS`)                                                           | `false`                 |
| `SERVER_SERVERLESS_OPERATOR_OPERATOR_NAME`             | Operator name serverless registrations attach their workers to                                                                                        | `serverless`            |
| `SERVER_SERVERLESS_OPERATOR_DEFAULT_SLOTS`             | Default slots per serverless registration worker                                                                                                      | `10000`                 |
| `SERVER_SERVERLESS_OPERATOR_DURABLE_SLOTS`             | Durable slots per serverless registration worker                                                                                                      | `10000`                 |
| `SERVER_SERVERLESS_OPERATOR_LEASE_TTL`                 | Serverless process lease TTL                                                                                                                          | `15s`                   |
| `SERVER_SERVERLESS_OPERATOR_HEARTBEAT_INTERVAL`        | Serverless process lease heartbeat interval                                                                                                           | `5s`                    |
| `SERVER_SERVERLESS_OPERATOR_REBALANCE_INTERVAL`        | Serverless lease claim/shed tick                                                                                                                      | `5s`                    |
| `SERVER_SERVERLESS_OPERATOR_SHED_HYSTERESIS`           | Fraction above fair share tolerated before shedding serverless units                                                                                  | `0.2`                   |
| `SERVER_SERVERLESS_OPERATOR_DRAIN_TIMEOUT`             | Max time to await in-flight serverless deliveries on unit loss or shutdown                                                                            | `60s`                   |
| `SERVER_SERVERLESS_OPERATOR_ROUTING_REFRESH_INTERVAL`  | Serverless routing cache refresh interval                                                                                                             | `10s`                   |
| `SERVER_SERVERLESS_OPERATOR_HEALTHCHECK_TIMEOUT`       | Timeout of one serverless endpoint healthcheck                                                                                                        | `10s`                   |
| `SERVER_SERVERLESS_OPERATOR_HEALTHCHECK_CONCURRENCY`   | Max concurrent serverless endpoint healthchecks                                                                                                       | `256`                   |
| `SERVER_SERVERLESS_OPERATOR_WS_MAX_FRAME_BYTES`        | Max websocket frame size for durable serverless invocations                                                                                           | `4194304`               |
| `SERVER_SERVERLESS_OPERATOR_WS_PING_INTERVAL`          | Websocket ping interval for durable serverless invocations                                                                                            | `15s`                   |
| `SERVER_SERVERLESS_OPERATOR_ALLOW_EMPTY_INFRA_CIDRS`   | Start the in-engine serverless operator without infra blocked CIDRs                                                                                   | `false`                 |
| `SERVER_SERVERLESS_OPERATOR_INSECURE_DESTINATIONS`     | Disable the serverless delivery SSRF policy (local development only)                                                                                  | `false`                 |
...
| `K8S_CLOUD_REGION`                       | Sets the `cloud.region` OpenTelemetry resource attribute. Falls back to `AWS_REGION` or `AWS_DEFAULT_REGION` if unset                  |               |
