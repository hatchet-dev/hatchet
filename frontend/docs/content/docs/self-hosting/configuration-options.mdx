...
Hatchet uses the standard `sub` claim as the stable user identifier. Providers using group mappings must return the same `sub` from both the ID token and UserInfo endpoint.
...
Hatchet always requests the `groups` scope. When group mappings exist, the provider must include a `groups` claim in the ID token and UserInfo response. Memberships are reconciled from current UserInfo claims on authenticated dashboard requests.

Tenant admins can manage tenant-specific mappings under **Tenant Settings > Members** using the `ADMIN`, `MEMBER`, or `VIEWER` roles. Mappings apply only to the currently configured issuer. A group managed by `SERVER_AUTH_OIDC_GROUP_MAPPINGS` cannot be overridden for an individual tenant.
