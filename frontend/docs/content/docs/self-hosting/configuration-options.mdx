...
| `DATABASE_TENANT_POOL_MAX_PERCENT` | Percent of `DATABASE_MAX_CONNS` one tenant may hold. `100` installs no cap. Values from `1` to `99` cap every tenant at that percent of the pool. | `100` |
