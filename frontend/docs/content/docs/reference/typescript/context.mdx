...
| --------- | ---------------- | ------------------------------------------------------------------ |
| `action`  | `Action`         | The action assigned to the task.                                   |
| `runtime` | `ContextRuntime` | The runtime the context performs engine-facing operations through. |
...
| --------- | ------------------------------------------------------------- | -------------------------------- |
| `action`  | `Action`                                                      | The action assigned to the task. |
| `v1`      | [`HatchetClient`](/reference/typescript/client#hatchetclient) | The worker's Hatchet client.     |
| `worker`  | `InternalWorker`                                              | The worker running the task.     |
...
| ------------------------------------------------- | ------------------------------------------------------------- | ------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| <a id="property-runtime"></a> `runtime`           | `ContextRuntime`                                              | `undefined`   | The runtime the context performs engine-facing operations through.                                                                                                         |
| <a id="property-spawnindex"></a> ~~`spawnIndex`~~ | `number`                                                      | `0`           | **Deprecated** Kept for backward compat; prefer nextChildIndex.                                                                                                            |
| <a id="property-v1"></a> `v1`                     | [`HatchetClient`](/reference/typescript/client#hatchetclient) | `undefined`   | The Hatchet client of the worker running the task. Only set when the context was created by a worker; a runtime without a client (see ContextRuntime) leaves it undefined. |
...
| Parameter  | Type                                                                                                                                                                                 | Description                                                                 |
| ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------- |
| `workflow` | \| `string` \| `BaseWorkflowDeclaration`\<`Q`, `P`\> \| [`TaskWorkflowDeclaration`](/reference/typescript/runnables#taskworkflowdeclaration)\<`Q`, `P`, \{ \}, \{ \}, \{ \}, \{ \}\> | The workflow to run (name, Workflow instance, or WorkflowV1 instance).      |
...
The triggering event external ID, or undefined if the workflow was not triggered by an event.
...
The triggering event key, or undefined if the workflow was not triggered by an event.
...
This method returns the task name. Use [workflowNameV1](#workflownamev1) for the workflow
name or [taskName](#taskname) for the task name.

<a id="workflownamev1"></a>
...
| `action`    | `Action`                           | The action assigned to the task.                                   |
| `runtime`   | `ContextRuntime`                   | The runtime the context performs engine-facing operations through. |
| `transport` | `DurableTransport`                 | The transport durable events travel over.                          |
| `options?`  | `DurableContextOptions` & `object` | Engine version and eviction settings.                              |

Returns

[`DurableContext`](#durablecontext)\<`T`, `K`\>

Overrides

[`Context`](#context).[`constructor`](#constructor)
...
| `v1`               | [`HatchetClient`](/reference/typescript/client#hatchetclient) | The worker's Hatchet client.                                      |
| `worker`           | `InternalWorker`                                              | The worker running the task.                                      |
| `durableListener`  | `DurableListenerClient`                                       | The worker's durable listener.                                    |
| `evictionManager?` | `DurableEvictionManager`                                      | The worker's eviction manager, when the engine supports eviction. |
| `engineVersion?`   | `string`                                                      | The engine version the worker is connected to.                    |
...
| `duration`         | `Duration` | The duration to sleep for.                                                      |
| `readableDataKey?` | `string`   | -                                                                               |
| `label?`           | `string`   | Optional wait label used when readableDataKey is passed as the second argument. |
...
| Parameter  | Type                                                                                                                                                                                 | Description                              |
| ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------- |
| `workflow` | \| `string` \| `BaseWorkflowDeclaration`\<`Q`, `P`\> \| [`TaskWorkflowDeclaration`](/reference/typescript/runnables#taskworkflowdeclaration)\<`Q`, `P`, \{ \}, \{ \}, \{ \}, \{ \}\> | The workflow to spawn.                   |
...
| `payloadSchema?`  | `undefined` | An optional Zod schema to validate and parse the event payload. |
| `scope?`          | `string`    | -                                                               |
