...
| `list`                                       | List task runs according to a set of filters, paginating through days |
| `aio_list`                                   | List task runs according to a set of filters, paginating through days |
...
| Type                  | Description                                         |
| --------------------- | --------------------------------------------------- |
| `list[V1TaskSummary]` | A list of task runs matching the specified filters. |
...
IMPORTANT: It's preferable to use `Workflow.run` (and similar) to trigger workflows if possible. This method is intended to be an escape hatch. For more details, see [the documentation](/reference/python/runnables#workflow).
