...
#### `binary_path = None` `class-attribute` `instance-attribute`

Path to an existing sidecar binary. When set, the download is skipped.

#### `checksum = None` `class-attribute` `instance-attribute`
...
#### `database_url = None` `class-attribute` `instance-attribute`

Connection string for an existing Postgres to use instead of the bundled one.

#### `postgres_data_dir = None` `class-attribute` `instance-attribute`
...
#### `grpc_port = None` `class-attribute` `instance-attribute`

Override the port the embedded engine's gRPC server listens on.

#### `api_port = None` `class-attribute` `instance-attribute`

Override the port the embedded REST API listens on.

#### `start_api = True` `class-attribute` `instance-attribute`

Set to `False` to start only the engine and gRPC server, without the REST API.

#### `run_migrations = True` `class-attribute` `instance-attribute`

Set to `False` to skip running database migrations on startup.

#### `rabbitmq_url = None` `class-attribute` `instance-attribute`

Connection string for a RabbitMQ instance to use as the message queue instead of Postgres.

#### `log_level = None` `class-attribute` `instance-attribute`

Log level for the embedded engine.

#### `ready_timeout_seconds = DEFAULT_READY_TIMEOUT_SECONDS` `class-attribute` `instance-attribute`
