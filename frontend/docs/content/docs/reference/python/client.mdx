...
| `concurrency`                 | `int \| ConcurrencyExpression \| Sequence[ConcurrencyExpression] \| None` | A concurrency object controlling the concurrency settings for this workflow. If an integer is provided, it is treated as a constant concurrency limit with a `GROUP_ROUND_ROBIN` strategy, which means that only `N` runs of the task may execute at any given time.              | `None`           |
...
| `concurrency`                 | `int \| ConcurrencyExpression \| Sequence[ConcurrencyExpression] \| None` | A concurrency object controlling the concurrency settings for this task. If an integer is provided, it is treated as a constant concurrency limit with a `GROUP_ROUND_ROBIN` strategy, which means that only `N` runs of the task may execute at any given time. | `None`                  |
