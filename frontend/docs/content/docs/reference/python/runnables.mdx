...
| `concurrency`           | `int \| Sequence[ConcurrencyExpression \| SharedConcurrency] \| None` | A list of concurrency expressions for the task. If an integer is provided, it is treated as a constant concurrency limit with a `GROUP_ROUND_ROBIN` strategy, which means that only `N` runs of the task may execute at any given time.      | `None`                  |
