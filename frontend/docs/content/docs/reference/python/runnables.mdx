...
| `schedule_timeout` | `timedelta` | The maximum time to wait for the task to be scheduled. The run will be canceled if the task does not begin within this time. | `timedelta(minutes=5)` |
| `execution_timeout` | `timedelta` | The maximum time to wait for the task to complete. The run will be canceled if the task does not complete within this time. | `timedelta(seconds=60)` |
...
| `desired_worker_labels` | `list[DesiredWorkerLabel] \| None` | A list of desired worker labels that determine to which worker the task should be assigned. See documentation and examples on affinity and worker labels for more details. | `None` |
...
| `concurrency` | `int \| Sequence[ConcurrencyStrategy] \| None` | A list of concurrency expressions for the task. If an integer is provided, it is treated as a constant concurrency limit with a `GROUP_ROUND_ROBIN` strategy, which means that only `N` runs of the task may execute at any given time. | `None` |
...
| `backoff_factor` | `float \| None` | The backoff factor for controlling exponential backoff in retries. | `None` |
| `backoff_max_seconds` | `int \| None` | The maximum number of seconds to allow retries with exponential backoff to continue. | `None` |
| `concurrency` | `int \| Sequence[ConcurrencyStrategy] \| None` | A list of concurrency expressions for the on-failure task. If an integer is provided, it is treated as a constant concurrency limit with a `GROUP_ROUND_ROBIN` strategy, which means that only `N` runs of the task may execute at any given time. | `None` |
...
| `backoff_factor` | `float \| None` | The backoff factor for controlling exponential backoff in retries. | `None` |
| `backoff_max_seconds` | `int \| None` | The maximum number of seconds to allow retries with exponential backoff to continue. | `None` |
| `concurrency` | `int \| Sequence[ConcurrencyStrategy] \| None` | A list of concurrency expressions for the on-success task. If an integer is provided, it is treated as a constant concurrency limit with a `GROUP_ROUND_ROBIN` strategy, which means that only `N` runs of the task may execute at any given time. | `None` |
...
| Name | Type | Description | Default |
| --- | --- | --- | --- |
| `input` | `TWorkflowInput` | The input data for the workflow, must match the workflow's input type. | `cast('TWorkflowInput', None)` |
...
| Type | Description |
| --- | --- |
| `WorkflowRunRef \| dict[str, Any]` | The result of the workflow execution as a dictionary, or a WorkflowRunRef if wait\_for\_result is False. |
...
| Type | Description |
| --- | --- |
| `list[dict[str, Any]] \| list[dict[str, Any] \| BaseException] \| list[WorkflowRunRef]` | A list of results for each workflow run, or a list of WorkflowRunRef if wait\_for\_result is False. |
...
| `input` | `TWorkflowInput` | The input data for the workflow. | `cast('TWorkflowInput', None)` |
| `child_key` | `str \| None` | An optional key for deduplicating child workflow runs. | `None` |
| `additional_metadata` | `JSONSerializableMapping \| None` | Additional metadata to attach to the workflow run. | `None` |
| `priority` | `Priority \| None` | The priority of the scheduled workflow run. | `None` |
...
| `priority` | `Priority \| None` | The priority of the cron job. | `None` |
...
| `dependencies` | `dict[str, Any] \| None` | Dependencies to be injected into the task. This is useful for tasks that have dependencies defined using `Depends`. **IMPORTANT**: You must pass the dependencies *directly*, **not** the `Depends` objects themselves. For example, if you have a task that has a dependency `config: Annotated[str, Depends(get_config)]`, you should pass `dependencies={"config": "config_value"}` to `aio_mock_run`. | `None` |
...
| Type | Description |
| --- | --- |
| `TaskRunRef[TWorkflowInput, R] \| R` | The extracted result of the workflow execution, or a TaskRunRef if wait\_for\_result is False. |
...
| Type | Description |
| --- | --- |
| `list[R] \| list[R \| BaseException] \| list[TaskRunRef[TWorkflowInput, R]]` | A list of results for each workflow run, or a list of TaskRunRef if wait\_for\_result is False. |
...
| Name | Type | Description | Default |
| --- | --- | --- | --- |
| `run_id` | `str` | The ID of the run to get the reference for. | *required* |
...
| Name | Type | Description | Default |
| --- | --- | --- | --- |
| `run_id` | `str` | The ID of the run to get the result for. | *required* |
