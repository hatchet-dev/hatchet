...
- **A full API client**: the [resource commands](/cli/managing-resources) cover everything an admin API token can do — runs, workflows, workers, events, filters, crons, scheduled runs, rate limits, webhooks, logs, API tokens, tenant settings, members, invites, and alerting — with JSON output for scripting.
