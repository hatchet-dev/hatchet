...
- **Locally served dashboard**: the [`hatchet embed-ui`](/cli/embed-ui) command serves the Hatchet dashboard from the CLI for an embedded Hatchet instance, so you can browse an instance that ships no frontend without deploying the dashboard separately.
