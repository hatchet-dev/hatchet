...
The client exposes lazily-initialized [feature clients](/reference/go/feature-clients/cel) as methods:

#### `CEL()`

CEL returns a client for working with CEL expressions. See the [CEL client](/reference/go/feature-clients/cel).

#### `Crons()`

Crons returns a client for managing cron triggers. See the [Crons client](/reference/go/feature-clients/crons).

#### `Filters()`

Filters returns a client for managing event filters. See the [Filters client](/reference/go/feature-clients/filters).

#### `Logs()`

Logs returns a client for managing task logs. See the [Logs client](/reference/go/feature-clients/logs).

#### `Metrics()`

Metrics returns a feature client for interacting with workflow and task metrics. See the [Metrics client](/reference/go/feature-clients/metrics).

#### `RateLimits()`

RateLimits returns a client for managing rate limits. See the [Rate Limits client](/reference/go/feature-clients/ratelimits).

#### `Runs()`

Runs returns a client for managing workflow runs. See the [Runs client](/reference/go/feature-clients/runs).

#### `Schedules()`

Schedules returns a client for managing scheduled workflow runs. See the [Schedules client](/reference/go/feature-clients/schedules).

#### `Webhooks()`

Webhooks returns a client for managing webhooks. See the [Webhooks client](/reference/go/feature-clients/webhooks).

#### `Workers()`

Workers returns a client for managing workers. See the [Workers client](/reference/go/feature-clients/workers).

#### `Workflows()`

Workflows returns a client for managing workflow definitions. See the [Workflows client](/reference/go/feature-clients/workflows).
...
RunMany executes multiple workflow instances with different inputs. The returned results are in the same order as the inputs. Returns workflow run IDs that can be used to track the run statuses.
