...
Options for `WithEmbedded`, which runs a full Hatchet engine in-process for local development. See [Embedded Hatchet](/v1/embedded).
...
Helpers for building the conditions used with `WithWaitFor`, `WithSkipIf`, and `DurableContext.WaitFor` (see [Context](/reference/go/context)):
...
Requires a blank import of the hatchet-embedded package (github.com/hatchet-dev/hatchet-embedded), which registers the engine backend. See the [Embedded Hatchet guide](https://docs.hatchet.run/v1/embedded) for usage.
