...
| `NewDurableTask` | NewDurableTask transforms a typed function into a durable Hatchet task that runs as part of a workflow. |
| `NewTask`        | NewTask transforms a typed function into a Hatchet task that runs as part of a workflow.                |
| `OnFailure`      | OnFailure sets a typed failure handler for the workflow.                                                |
...
NewDurableTask transforms a typed function into a durable Hatchet task that runs as part of a workflow. Like NewTask, the input and output types are type parameters checked at compile time and invoked without reflection.
...
| --------- | ---------------------------------------------- |
| `name`    | `string`                                       |
| `fn`      | `func(ctx DurableContext, input I) (O, error)` |
| `options` | `...TaskOption`                                |
...
The input and output types are captured as type parameters, so the function signature is checked at compile time and invoked directly — no reflection. Type parameters are inferred from the function literal, so existing call sites need no changes.
...
OnFailure sets a typed failure handler for the workflow. It runs when any task in the workflow fails and receives the same input the workflow was triggered with. The input and output types are type parameters, inferred from the handler and invoked without reflection.
...
| `OnFailure` | OnFailure sets a typed failure handler for the standalone task.                                |
...
OnFailure sets a typed failure handler for the standalone task. It runs when the task fails and receives the same input the task was triggered with. The input and output types are type parameters, inferred from the handler and invoked without reflection.
