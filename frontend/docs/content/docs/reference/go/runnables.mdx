...
| `UserEventCondition` | `UserEventCondition(eventKey, expression string)` | UserEventCondition creates a condition that waits for a user event.                             |
...
| `ErrorWindow` | `time.Duration` | ErrorWindow is how long after start the notice is an error log. After this window, calls have a 20% chance of returning an error. If zero (default), the error/raise phase is never reached — the notice stays at error-level logging indefinitely. |
