...
BatchInputInto decodes the buffered items of a batch task's START\_BATCH action into target, which must be a pointer. The decoded value is a map keyed by each buffered item's task run external id.
