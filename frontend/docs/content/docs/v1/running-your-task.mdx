...
The sync `run` method polls for completion every 1 second by default. Configure the interval with `ClientConfig.sync_result_poll_interval` or `HATCHET_CLIENT_SYNC_RESULT_POLL_INTERVAL`. The minimum is 1 second. `aio_run` waits on a listener and is not affected.
