...
The sync `run` method polls for completion every 1 second by default. Configure the interval with `ClientConfig.sync_result_poll_interval` or the `HATCHET_CLIENT_SYNC_RESULT_POLL_INTERVAL` environment variable. The minimum is 1 second. `aio_run` waits on a listener and is not affected.
...
`result()` uses the same poll interval as sync `run`. Pass `poll_interval` to override it for a single call. `aio_result` waits on a listener and is not affected.
