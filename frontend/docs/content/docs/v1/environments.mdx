...
import { Bleed } from "@/components/nextra-compat";
...
If you are using Hatchet locally, you can create a local instance of Hatchet to manage your isolated local development environment.

For development and testing, the fastest option is [embedded mode](/v1/embedded). It runs a full Hatchet engine from inside your worker process, with no account, token, or Docker required.

For a production-like local stack with the dashboard and separate services, run [Hatchet Lite](/self-hosting/hatchet-lite).
