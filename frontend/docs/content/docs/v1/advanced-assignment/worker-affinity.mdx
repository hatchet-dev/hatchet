...
```
  ...
  <Tabs.Tab title="Ruby">
    <Snippet src={snippets.ruby.affinity_workers.worker.affinity_workflow} />
  </Tabs.Tab>
</UniversalTabs>

<Callout type="warning">
  Use extra care when using worker affinity with [sticky assignment `HARD`
  strategy](./sticky-assignment). In this case, it is recommended to set desired
  labels on the first task of the workflow to ensure that the workflow is
  assigned to a worker that meets the desired criteria and remains on that
...
<Snippet src={snippets.typescript.affinity.affinity_workers.task_with_labels} />
...
```
  ...
  <Tabs.Tab title="Ruby">
    <Snippet src={snippets.ruby.affinity_workers.worker.affinity_task} />
  </Tabs.Tab>
</UniversalTabs>
