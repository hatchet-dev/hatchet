...
import { EmbeddedMode } from "@/components/flow-animations/EmbeddedMode";
...
<Keywords keywords="embedded, local, local development, local testing, ci, in-process, no docker, without docker, embedded postgres, sidecar" />
...
This opens a browser to the locally served dashboard, already signed in. The URL contains a `ui_token` that is exchanged for a local session cookie and stays valid while the command runs; requests without the cookie are rejected, so other processes or machines cannot use the proxy port. The server binds to `localhost` unless you override `--host`.

There is no login screen: the proxy signs the browser in to the embedded instance automatically as the seeded admin user (email `admin@example.com`, password `Admin123!!`) and re-establishes the session transparently when it expires. The command does not print the credentials; its startup output only says where they come from (the seed defaults, or your environment overrides), and this page documents the defaults in case you want to sign in manually. If you seeded a different admin (with `WithAdminUser`, or by setting `ADMIN_EMAIL` and `ADMIN_PASSWORD` on the embedded process), run `hatchet embedded-ui` with the same `ADMIN_EMAIL` and `ADMIN_PASSWORD` environment variables so the automatic sign-in uses those credentials. If you sign in manually as a different user, the proxy respects that session while it remains valid and only signs in as the admin again once it expires.
