...
Run a full Hatchet engine from inside your application, with no Docker and no external services. [hatchet-embedded](https://github.com/hatchet-dev/hatchet-embedded) starts the engine, seeds an admin tenant, and hands the SDK a client wired to the embedded instance.
...
<Callout type="warning">
  Embedded mode is designed for local development and testing only.
</Callout>
...
Embedded mode lives in its own module (`hatchet_sdk.embedded`), so nothing loads unless you import it:
...
```python
from hatchet_sdk.embedded import EmbeddedOptions, HatchetEmbedded
...
<Callout type="warning">
  ...
  `HatchetEmbedded()` handles this automatically, and subprocesses connect to
