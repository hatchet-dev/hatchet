...
This guide migrates every Temporal workflow to a durable task first, because that translation is mechanical and preserves the semantics you have today. Once that is running, many of those durable tasks turn out not to need durable execution at all: Hatchet is a general-purpose task queue, and a workflow that never waits is better expressed as a plain task or a DAG. [Step 13](#step-13-simplify-once-the-migration-is-green) covers that clean-up, after the migration is green.
...
| [13](#step-13-simplify-once-the-migration-is-green)     | A workflow that is a fixed sequence, or a wrapper | A `parents=[...]` DAG, or a plain task                    | Simplification      |
...
Every Temporal workflow becomes a Hatchet durable task, and the translation is mechanical. The workflow body becomes the durable task body, and each `execute_activity(...)` call becomes a call to the Hatchet task you defined in step 3. Semantics are preserved and nothing is redesigned, so you can convert an entire codebase this way without making a single architectural decision.
...
Everything that made this a workflow is still true of the durable task. It has a run id and a history, it survives a worker crash, and it resumes from its last checkpoint rather than from the top. What changes is local to each line. `workflow.execute_activity(fn, arg, ...)` becomes `fn.aio_run(input)` (`.run(...)` in TypeScript, Go and Ruby), results come back as ordinary return values rather than futures, and the per-call retry and timeout options move onto the task definition instead, which is [step 6](#step-6-retries-and-timeouts).
...
<Callout type="info">
  ...
  then see [step 13](#step-13-simplify-once-the-migration-is-green).
</Callout>
## Step 5: Invoke work
...
A workflow that spawns one child per item becomes a durable task that runs one child task per item:
...
<Callout type="info">
  ...
  Ruby, crons have to be declared on a workflow rather than on a standalone
  task, which is what the Ruby example above does; the TypeScript example uses a
  workflow too, though a standalone TypeScript task accepts an `on.cron` option
  as well.
</Callout>
...
- Most of what was a Temporal workflow can end up as a plain task or a DAG in Hatchet (see [step 13](#step-13-simplify-once-the-migration-is-green)), and those have no replay history to stay compatible with. You can change their bodies freely.
...
## Step 13: Simplify once the migration is green

Step 4 converted every workflow the same way, on purpose. Now that those durable tasks are running and passing their tests, two kinds of them can be simplified further. Both are optional. Neither changes what your code does, so nothing here has to happen on the migration's critical path.

<Callout type="warning">
  Do this only once the 1:1 migration is green. These are refactors of working
  code, not migration steps. Doing them while you are still chasing behavioural
  differences makes it much harder to tell which change broke what.
</Callout>
