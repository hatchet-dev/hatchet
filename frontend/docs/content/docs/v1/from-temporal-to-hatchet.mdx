...
Second, workers in Hatchet have a built-in notion of concurrency limits, instead of per-activity or per-workflow rate limits. This makes it very easy to reason about the amount of work that a worker is able to accept.
...
# Hatchet
await hatchet.event.aio_push("approval:granted", {"order_id": "123"})
```
...
// Hatchet
await hatchet.events.push("approval:granted", { orderId: "123" });
```
...
# Hatchet
HATCHET.events.push("approval:granted", { "order_id" => "123" })
```
...
async def approval_flow(input: OrderInput, ctx: DurableContext) -> None:
    await ctx.aio_wait_for_event("approval:granted", f"input.order_id == '{input.order_id}'")
```
...
approvalFlow := client.NewStandaloneDurableTask("ApprovalFlow",
	func(ctx hatchet.DurableContext, input OrderInput) (ApprovalOutput, error) {
		expr := fmt.Sprintf("input.order_id == '%s'", input.OrderID)
...
APPROVAL_FLOW = HATCHET.durable_task(name: "ApprovalFlow") do |input, ctx|
  ctx.wait_for(
    ...
    Hatchet::UserEventCondition.new(
      ...
      expression: "input.order_id == '#{input['order_id']}'"
    )
  )
end
```
...
- **Updates** — a synchronous, validated mutation of running workflow state — have no equivalent. Model this as an event the durable task waits on, plus a child task that performs the mutation and returns a result.
...
Hatchet places no cap on the number of children spawned from a durable run. Temporal enforces a limit on pending child executions and pending activities per run — 51,200 by default, tunable in server dynamic config — which is the constraint behind most Temporal "continue-as-new to keep the history small" batching code. That batching code can usually be deleted; for large fan-outs, see [bulk run](/v1/bulk-run).
