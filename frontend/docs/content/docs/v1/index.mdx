...
- **Minimal Infra Dependencies** - Hatchet is built on top of PostgreSQL and for simple workloads, [it's all you need](/self-hosting/hatchet-lite). For local development, [embedded mode](/v1/embedded) runs the full engine inside your worker process with no Docker at all.
