...
<Callout type="warning">
  ...
  will need to define a Pydantic model that captures the expected fields. Tasks
  that take no meaningful input can omit the `input_validator` and declare their
  input as `None`. This input-model conversion is the main mechanical cost of
  the migration.
</Callout>
