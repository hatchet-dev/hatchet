...
| [10](#step-10-migrate-other-celery-project-surfaces)       | `revoke` / task cancellation             | [Cancellation API](/v1/cancellation) + dashboard + `ctx.is_cancelled`            | Small rewrite       |
...
See [retry policies](/v1/retry-policies) for the full set of retry options and behavior.
...
Hatchet supports [task cancellation](/v1/cancellation) through the runs client, the dashboard, or concurrency strategies like `CANCEL_IN_PROGRESS`. To cancel a run programmatically, use `hatchet.runs.cancel(run_id)`. Running tasks should cooperate by checking `ctx.is_cancelled`:
...
- [Retry policies](/v1/retry-policies): retries and backoff
- [Timeouts](/v1/timeouts): execution and scheduling timeouts
