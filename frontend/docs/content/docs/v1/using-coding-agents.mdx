...
<Callout type="info">
  **Prerequisite:** The `hatchet skills install` and `hatchet mcp install`
  commands require the Hatchet CLI. See the <a href="/cli">CLI reference</a> for
...
Access is default-deny, you must grant access to [CLI profiles](/cli/profiles) during the installation process or with `hatchet mcp auth` after installation. The only exception is [embedded instance](/v1/embedded), which is always available without an explicit grant.

See the [MCP server reference](/cli/mcp) for all targets, scopes, and the grants model.
