...
This creates a `.agents/skills/hatchet-cli/` directory with step-by-step reference files, links it from `.claude/skills/hatchet-cli` and `.cursor/skills/hatchet-cli` so agents that auto-discover skills pick it up, and adds a section to your project's `AGENTS.md` (and `CLAUDE.md`) pointing agents to the right file for each task.
...
After installation, commit the `.agents/`, `.claude/`, and `.cursor/` directories along with `AGENTS.md` to version control so all agents working in the repo benefit automatically.

If you installed an earlier version of the skill package to `skills/hatchet-cli/`, rerunning `hatchet skills install` moves the install to `.agents/skills/hatchet-cli/` and removes the old directory.
