...
description: "Historical guide to upgrading a self-hosted Hatchet engine from v0 to v1. v0 has been removed, so this page is kept for reference only."
---
...
- [Python SDK Migration Guide](/v1/migrating/migration-guide-python)
- [Typescript SDK Migration Guide](/v1/migrating/migration-guide-typescript)
- [Go SDK Migration Guide](/v1/migrating/migration-guide-go)
...
- We will only be supporting [CEL-based concurrency keys](/v1/concurrency), and we will not be supporting custom concurrency methods defined on the client. If you require custom logic to compute the concurrency key that can't be captured in a CEL expression, we recommend computing the key ahead of time and passing it as part of the input to the workflow. **Workflows registered against a v1 engine with a custom concurrency method (instead of an expression) will not use a concurrency queue.**
