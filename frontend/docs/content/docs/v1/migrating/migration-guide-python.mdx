...
2. External-facing protobuf objects, such as `StickyStrategy`, have been replaced by native Python enums to make working with them easier. Concurrency strategies are passed as a plain string `Literal` on `ConcurrencyStrategy` rather than a separate enum type.
