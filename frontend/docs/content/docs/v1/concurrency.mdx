...
- [**Cancel Except Newest**](#cancel-except-newest) cancels any incoming task or workflow runs for a key once it has reached a provided limit, except for the most recently enqueued.
- [**Cancel Except Oldest**](#cancel-except-oldest) cancels any incoming task or workflow runs for a key once it has reached a provided limit, except for the least recently enqueued.
...
When a new task instance is triggered, the Cancel Except Newest strategy will:
...
Cancel Except Newest is very similar to Cancel Newest, except that it will keep `max_runs` of the most recent enqueued tasks available until the in-progress tasks are done running.
...
To use this strategy, set the `CANCEL_EXCEPT_NEWEST` limit strategy along with a `max_runs` limit and a key expression:

<UniversalTabs items={["Python", "Typescript", "Go", "Ruby"]}>
  <Tabs.Tab title="Python">
    <Snippet
      src={
        snippets.python.concurrency_cancel_except_newest.worker
          .cancel_except_newest
      }
    />
  </Tabs.Tab>
  <Tabs.Tab title="Typescript">
    <Snippet
      src={
        snippets.typescript.concurrency_cancel_except_newest.workflow
          .cancel_except_newest
      }
    />
  </Tabs.Tab>
  <Tabs.Tab title="Go">
    <Snippet src={snippets.go.concurrency.main.cancel_except_newest} />
  </Tabs.Tab>
  <Tabs.Tab title="Ruby">
    <Snippet
      src={
        snippets.ruby.concurrency_cancel_except_newest.worker
          .cancel_except_newest
      }
    />
  </Tabs.Tab>
...
When a new task instance is triggered, the Cancel Except Oldest strategy will:
...
Cancel Except Oldest is the opposite of Cancel Except Newest: instead of queuing the most recently enqueued tasks, it cancels newer tasks, and keeps `max_runs` of the earliest tasks available until the in-progress tasks are done running.
...
To use this strategy, set the `CANCEL_EXCEPT_OLDEST` limit strategy along with a `max_runs` limit and a key expression:

<UniversalTabs items={["Python", "Typescript", "Go", "Ruby"]}>
  <Tabs.Tab title="Python">
    <Snippet
      src={
        snippets.python.concurrency_cancel_except_oldest.worker
          .cancel_except_oldest
      }
    />
  </Tabs.Tab>
  <Tabs.Tab title="Typescript">
    <Snippet
      src={
        snippets.typescript.concurrency_cancel_except_oldest.workflow
          .cancel_except_oldest
      }
    />
  </Tabs.Tab>
  <Tabs.Tab title="Go">
    <Snippet src={snippets.go.concurrency.main.cancel_except_oldest} />
  </Tabs.Tab>
  <Tabs.Tab title="Ruby">
    <Snippet
      src={
        snippets.ruby.concurrency_cancel_except_oldest.worker
          .cancel_except_oldest
      }
    />
  </Tabs.Tab>
