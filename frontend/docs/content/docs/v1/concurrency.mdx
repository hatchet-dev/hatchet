...
Concurrency control also lets you limit the number of runs for a workflow globally, if you use a static CEL expression, such as `'global'`. This and [rate limiting](./rate-limits) are the recommended mechanisms for setting per-workflow throughput limits.
...
<Callout type="info">
  ...
  slots than a light one, see [Task Slot Cost](./advanced-assignment/slot-cost).
</Callout>
