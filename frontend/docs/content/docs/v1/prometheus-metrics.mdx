...
<Callout type="info">
  Only available in the Enterprise tier on Hatchet Cloud, [reach
...
<Callout type="info">
  For a full list of available metrics, setup instructions, and example PromQL
...
The response is returned in standard Prometheus Text Format. Every metric name
gets a `# TYPE <name> untyped` line followed by its samples with labels, values
and millisecond timestamps. Metric types and descriptions are listed in the
[tenant metrics table](/self-hosting/prometheus-metrics#tenant-metrics). Series
are summed across engine pods, so each label set appears once.
