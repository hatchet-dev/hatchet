...
<Callout type="info">
  **Just developing locally?** You can run Hatchet with no account, no API
  token, and no Docker using <a href="/v1/embedded">embedded mode</a>. It runs a
  full Hatchet engine from inside your worker process.
</Callout>
