...
function SsoSetupDialogWrapper() {
    const { loading, hasUnsavedChanges, reset, isOpen, setIsOpen, hasExistingConfig } = useSsoSetupFormMetadata();
        ...
        <Dialog open={isOpen} onOpenChange={handleOpenChange} modal={true}>
            <DialogTrigger asChild>
                <Button variant="outline" disabled={loading} className="cursor-pointer">
                    ...
                    {loading ? "Loading..." : hasExistingConfig ? "Edit SSO Config" : "Set up SSO"}
                </Button>
            </DialogTrigger>
            <DialogContent
    ...
    return (
        <DialogHeader>
            <DialogTitle>
                ...
                {step === SsoSetupStep.Configuration && ` - ${PROVIDER_CONFIG[provider].displayName}`}
            </DialogTitle>
            {step === SsoSetupStep.ProviderSelection && (
                <DialogDescription>Choose your Identity Provider to get started.</DialogDescription>
            )}
...
function SsoFormActions() {
    const { step, existing, saving, onBack, onSave, onDeleteClick } = useSsoFormActions();

    if (step !== SsoSetupStep.Configuration) return null;
    ...
    return (
        <div className="flex w-full items-center justify-between mt-6">
            ...
            <div className="flex items-center gap-2 ml-auto">
                {existing && (
                    <Button variant="destructive" onClick={onDeleteClick} className="cursor-pointer">
                        Delete
                    </Button>
                )}
