...
const getDate = (d: MinuteEntry) => d.date;
const getValue = (d: MinuteEntry) => d.value;
const getYValue = (d: MinuteEntry) => {
...
const bisectDate = bisector<MinuteEntry, Date>((d) => d.date).left;


type TooltipData = MinuteEntry;
...
export default withTooltip<MultiBarChartProps, TooltipData>(
  ({
    data,
    kind,
    ...
    tooltipTop = 0,
    tooltipLeft = 0,
  }: MultiBarChartProps & WithTooltipProvidedProps<TooltipData>) => {
    if (width < 10) {
      return null;
    }
              ...
              data.map((d, i) => {
                if (i == 0) {
                  return (
                    ...
                    <Bar key={i}
                     // change x and color and fill


                      x={dateScale(getDate(d)) + getYValue(d) }
                      y={yScale(getValue(d)) }
                      width={(barWidth - 4) / 2}
                ...
                return (
                   ...
                   <Bar key={i}
                    ...
                    x={(dateScale(getDate(d))  + getYValue(d) ) - barWidth / 2}
                    y={yScale(getValue(d)) || 0}
                    width={barWidth - 4}
                    height={innerHeight - yScale(getValue(d)) }
                    fill={getFill(d)}
                    rx={2}
                  />
