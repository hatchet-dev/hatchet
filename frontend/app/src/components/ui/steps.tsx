...
const Step = ({
  ...
  const [open, setOpen] = React.useState(!disabled || !defaultCollapsed);

  // if disabled becomes true, close the step
  React.useEffect(() => {
    if (disabled) {
      setOpen(false);
    }
  ...
  return (
    ...
    >
      <AccordionItem value="open">
        <AccordionTrigger
          hideChevron={!collapsible || disabled}
          className={
            !collapsible || disabled ? 'hover:no-underline cursor-default' : ''
          }
        >
