...
const createGenericSchema = createBaseSchema.extend({
    provider: z.union([z.literal("Generic"), z.literal("Google"), z.literal("OneLogin"), z.literal("JumpCloud")]),
    authUrl: z.url({ message: "Must be a valid URL" }),
    tokenUrl: z.url({ message: "Must be a valid URL" }),
    userinfoUrl: z.url({ message: "Must be a valid URL" }),
});
...
export const editFormSchema = z.discriminatedUnion("provider", [editOktaSchema, editEntraSchema, editGenericSchema]);
