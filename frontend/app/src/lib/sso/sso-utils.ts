...
export function normalizeSsoApi(api?: Partial<SsoApi>): SsoApi {
    return {
        async get() {
            ...
            console.error("SsoSetup: api.get not implemented. Pass an `api` prop to implement.");
            return { ok: true, data: {} };
        },
        async upsert(args) {
            if (api?.upsert) return api.upsert(args);
            const message = "SsoSetup: api.upsert not implemented. Pass an `api` prop to implement.";
            console.error(message);
            return { ok: false, error: { message } };
        },
        async remove() {
            if (api?.remove) return api.remove();
            const message = "SsoSetup: api.remove not implemented. Pass an `api` prop to implement.";
...
export function inferSsoProvider(info: IdpInfoFromCustomer): ProviderKey {
    if (info.idpType === "Okta") return "Okta";
    if (info.idpType === "MicrosoftEntra") return "MicrosoftEntra";
    return "Generic";
}

export function hydrateSsoForm(provider: ProviderKey, info: IdpInfoFromCustomer): FormValues {
    if (info.idpType === "Okta") {
        ...
        const genericProvider =
            provider === "Generic" || provider === "Google" || provider === "OneLogin" || provider === "JumpCloud"
                ? provider
                : "Generic";
        return {
...
export function toSsoIdpInfo(values: FormValues | EditFormValues): IdpInfoFromCustomer {
