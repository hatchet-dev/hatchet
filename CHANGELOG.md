...
- Tasks and workflows can declare an idempotency key, so duplicate triggers no longer produce duplicate runs. Keys are held for a TTL or until the claiming run reaches a terminal status, in all four SDKs.
- Tasks can declare a max batch size, a max interval, or both, and the engine dispatches the accumulated runs to a worker as a single invocation. A batch group key such as `input.tenant_id` partitions runs of the same task into separate batches.
