...
- Go SDK: `RunMany` returns results in the same order as its inputs ([#4825](https://github.com/hatchet-dev/hatchet/pull/4825)).
- Engine: worker `isActive` is guarded by a listener session id rather than a millisecond timestamp, closing a rare race where a reconnecting worker was marked inactive ([#4916](https://github.com/hatchet-dev/hatchet/pull/4916)).
