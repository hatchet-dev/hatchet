...
- Engine: satisfied durable event lookups are now filtered by tenant, enforcing tenant isolation on the durable event log ([GHSA-992g-9cr3-vm5x](https://github.com/hatchet-dev/hatchet/security/advisories/GHSA-992g-9cr3-vm5x)).
...
- `hatchet quickstart` supports use-case templates via a new `--use-case` flag, starting with `scheduled`: a Go template whose workflow runs on a cron schedule and can also be run on demand. See the [quickstart CLI docs](https://docs.hatchet.run/reference/cli/quickstarts).
...
- Hatchet Cloud now has self-serve support for tenant-scoped Prometheus metrics and an audit logs API available on Custom plans.
